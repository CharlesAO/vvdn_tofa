...
start(frameBuffer* inFrame, frameBuffer* outFrame, u32 use_builtins)
{
    if (use_builtins) {
        Effect_builtins(inFrame->p1, outFrame->p1, inFrame->spec.width, inFrame->spec.height);
    } else {
        Effect(inFrame->p1, outFrame->p1, inFrame->spec.width, inFrame->spec.height);
    }

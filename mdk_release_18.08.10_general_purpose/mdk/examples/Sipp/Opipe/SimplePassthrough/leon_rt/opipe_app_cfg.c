...
uint16_t iCircBuffCmx[I_CBUFF_H *IMG_W * 1] SECTION(".cmx.data") ALIGNED(16);  //10bit bayer
uint16_t oCircBuffCmx[O_CBUFF_H *IMG_W * 1] SECTION(".cmx.data") ALIGNED(16);  //10bit bayer
...
void buildAndCfg(Opipe *p, uint16_t *iBuf, uint16_t *oBuf)
{
    ...
    pIn = OpipeCfgBuff (p, SIPP_SIGMA_ID, D_DMA|D_IN, (uint32_t)iCircBuffCmx, I_CBUFF_H, BPP(2));
    pOut = OpipeCfgBuff (p, SIPP_SIGMA_ID, D_DMA|D_OUT,(uint32_t)oCircBuffCmx, O_CBUFF_H, BPP(2));

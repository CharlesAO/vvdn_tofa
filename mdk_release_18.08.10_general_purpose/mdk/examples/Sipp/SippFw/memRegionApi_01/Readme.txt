...
Overview
====================
This is a simple SIPP pipeline to test the SIPP framework API sippAllocCmxMemRegion() in
association with oPipe runtime.
The user has the option to verify that the regions touched are those specified in
SippMemRegion structure by using CHECK_TOUCHED_REGION define.
The slices that are allocated to the pipeline are initialized in main.c file and checked
that are not overwritten on test completion.

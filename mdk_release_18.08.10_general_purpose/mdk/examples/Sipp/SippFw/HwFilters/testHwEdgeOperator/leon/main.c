...
///
/// @brief EdgeOp filter unit test taken from c-model test
...
u8 DDR_DATA TestHwEdgeOperator_ref_buffer[SIPP_HW_EDGE_OP_BUFFER_SIZE];

TestHwEdgeOperator testEdgeOp;
...
int main (int argc, char *argv[])
{
    ...
    buildTestHwEdgeOperator (&testEdgeOp);

    if (sippPipeGetErrorStatus (testEdgeOp.pl))
    {
        u32 errNum;
    ...
    sippRegisterEventCallback (testEdgeOp.pl,
                               appSippCallback);
#endif

    configTestHwEdgeOperator (&testEdgeOp);

    readTestHwEdgeOperatorInput(&testEdgeOp);

#ifdef SIPP_USE_PRECALC_SCHEDULE
    // Read offline schedule decisions
    dbgSchedInit (testEdgeOp.pl);
#endif

#ifdef SIPP_TEST_SYNC_API
    sippProcessFrame (testEdgeOp.pl);
...
#else
    sippProcessFrameNB (testEdgeOp.pl);
    ...
    {                               //so this becomes one function call
    ...
    sippRdFileU8((u8*)TestHwEdgeOperator_ref_buffer,\
                 SIPP_HW_EDGE_OP_BUFFER_SIZE,\
                 "../../../../../../../resources/Ref_edgeOut_1296x972_P400.rgb");
    writeTestHwEdgeOperatorOutput(&testEdgeOp);
    sippDbgCompareU8((u8*)TestHwEdgeOperator_ref_buffer,\
                     (u8*)TestHwEdgeOperator_dmaOut0_buffer,\
                     SIPP_HW_EDGE_OP_BUFFER_SIZE);

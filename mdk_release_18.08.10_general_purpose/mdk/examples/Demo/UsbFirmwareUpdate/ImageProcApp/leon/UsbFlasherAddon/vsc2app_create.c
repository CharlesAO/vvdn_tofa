	...
	*/
	...
	if (StatusOk)
		...
		for (i = 0; i < USBPUMP_VSC2APP_NUM_REQUEST_IN; ++i)
			{
			pRequest->nBuffer = USBPUMP_VSC2APP_REQUEST_BUFSIZE_IN;
			pRequest->pBuffer =
				UsbAllocateDeviceBuffer(
					pSelf->pDevice,
					pRequest->nBuffer
					);
			if (pRequest->pBuffer == NULL)
				{
				TTUSB_PLATFORM_PRINTF((
					pPlatform,
					UDMASK_ERRORS | UDMASK_ANY,
					"?UsbPumpVsc2App_ClientCreate:"
					" can't allocate data buffer.\n"
					));
				StatusOk = FALSE;
				break;
				}
		...
		for (i = 0; i < USBPUMP_VSC2APP_NUM_REQUEST_OUT; ++i)
			{
			pRequest->nBuffer = USBPUMP_VSC2APP_REQUEST_BUFSIZE_OUT;
			pRequest->pBuffer =
				UsbAllocateDeviceBuffer(
					pSelf->pDevice,
					pRequest->nBuffer
...
UsbPumpVsc2App_OpenSession_Callback(
	...
	{
	USBPUMP_VSC2APP_CONTEXT * CONST	pSelf = pClientContext;
	...
	if (USBPUMP_PROTO_VSC2_STATUS_SUCCESS(Status))
		...
		if (! USBPUMP_PROTO_VSC2_STATUS_SUCCESS(Status))
			...
			TTUSB_PLATFORM_PRINTF((
				...
				UDMASK_ERRORS,
				"?UsbPumpVsc2App_OpenSession_Callback:"
				" failed to open bulk OUT stream.\n"
				));
			}
		}
	else

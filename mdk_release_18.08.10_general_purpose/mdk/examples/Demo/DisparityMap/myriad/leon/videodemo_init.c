...
extern u32 lrt_toSend;
u32 *pToSend;
volatile  u32 usbFrameCtr_prev = 1;
...
{
    UNUSED(wMaxPacketSize);
    UNUSED(wTransportHeaderSize);
...
)
{
	...
	*(pBuffer + UNCOMPRESSED_HDR_BFH_OFFSET) += UNCOMPRESSED_HDR_EOF;
	...
	//printf("0x%x\n", DrvTimerGetSysTicks64());
	//printf("U(%d) - %d \n", lrt_usbFrameCtr % MAX_USED_BUF, lrt_usbFrameCtr);

	/*if (cntWrite < 200)
		cyclesWrite[cntWrite++] = DrvTimerElapsedTicks(&timerWrite);

	DrvTimerStart(&timerWrite);*/
...
static void VideoDemoI_SendAnotherFrame(void *pClientContext)
{
	...
	/*if (cnt < 200)
		cycles[cnt++] = DrvTimerElapsedTicks(&timer_data);

	DrvTimerStart(&timer_data);*/

	status = LeonIPCWaitMessage(&lrt_LRTtoLOSChannel, IPC_WAIT_FOREVER);
    if (status)
    {
        printf("Could not initialize Leon IPC Rx. Error: %d\n", status);
        exit(1);

...
int initClocksAndMemory(void)
{
    ...
    for (int i = 0; i < 12; i++)
    {
        OsDrvShaveL2CSetNonWindowedPartition(i, partition_id, NON_WINDOWED_DATA_PARTITION);
        OsDrvShaveL2CSetNonWindowedPartition(i, partition_id, NON_WINDOWED_INSTRUCTIONS_PARTITION);
    }

    OsDrvShaveL2CachePartitionInvalidate(partition_id);
    return 0;
}

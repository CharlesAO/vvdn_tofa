...
void* sendAndReceiveMessages(void* temp) {
    //creating data to send
    int streamId = *(int*) temp;
    int linkId = (streamId >> 28) & 0xFFFF;
    srand(time(NULL));
    ...
    for(int i = 0; i<NUM_MESSAGES; i++) {
        ...
        //RECEIVE
        printf("Reading back data from link %d... ", linkId);
        fflush(stdout);
        status = XLinkReadData(streamId, &packet);
        if (status == X_LINK_SUCCESS)
        {
            printf("Data read from link %d\n", linkId);
        }
        else
        {
        ...
        if (packet->length != msgIdx[i+1]-msgIdx[i]){
            printf("Packet length wrong (linkid %d)\n", linkId);
            testPassed = false;
        }
        ...
        if (status != 0)
        {
            printf("release data  for link %d failed: %x\n", linkId, status);
            testPassed = false;
        }

    }
    return (void*) testPassed;
}

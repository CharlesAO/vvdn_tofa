...
print "\nConverting $network_name to cnnhwblob format and C code... patience please\n";
...
my $sourceCodeIncludeHeaders;
my $sourceCodeTempBuffers;
my $sourceCodeGenNetwork;
my $sourceCodeCopyFunction;

$sourceCodeIncludeHeaders .= "#include <stdio.h>\n";
$sourceCodeIncludeHeaders .= "#include <cstdlib>\n";
$sourceCodeIncludeHeaders .= "#include <cstring>\n";
$sourceCodeIncludeHeaders .= "#include \"HwModelsApi.h\"\n";

$sourceCodeCopyFunction .= "void copyToDDR(uint8_t *startOfDDR)\n{\n";
WriteBasicApiFunctionCalls();
ExtractCoefficients();
$sourceCodeCopyFunction .= "}\n";

open my $cfile_hnd, ">$network_name.c" or die;
print $cfile_hnd $sourceCodeIncludeHeaders;
print $cfile_hnd "cnnLayer *cnnLayers;\n";
print $cfile_hnd $sourceCodeTempBuffers;
print $cfile_hnd $sourceCodeCopyFunction;
print $cfile_hnd $sourceCodeGenNetwork;
close $cfile_hnd;
...
sub ExtractCoefficients
{
    ...
    foreach my $layer_ref (@layers)
    {
        ...
        if($layer{"stageType"} == 0 || $layer{"stageType"} == 4)
        {
            ...
            $sourceCodeIncludeHeaders .= "#include \"layer$layer_index.h\"\n";
            open my $hfile_hnd, ">layer".$layer_index.".h" or die;
            print $hfile_hnd "uint16_t weightsLayer_$layer_index"."[] = {\n";            
                    ...
                    for($ky = 0; $ky < $layer{"radixY"}; $ky++)
                    {
                        for($kx = 0; $kx < $layer{"radixX"}; $kx++)
                        {
                            ...
                            for($och = 0; $och < 8; $och++)
                            {
                                ...
                                print $hfile_hnd sprintf "0x%04X, ", unpack("v", $coeff);
                            }
                            print $hfile_hnd "\n";
                        }
                    }
            ...
            $sourceCodeCopyFunction .= "\tmemcpy(DDRweightsLayer_$layer_index = startOfDDR, weightsLayer_$layer_index, sizeof(weightsLayer_$layer_index)); startOfDDR = (uint8_t*)(((uint32_t)startOfDDR + sizeof(weightsLayer_$layer_index) + 0x1000) & 0xFFFFF000);\n";

            if($layer{"postOp"} == 6 || $layer{"postOp"} == 9)
            {                
                $sourceCodeCopyFunction .= "\tmemcpy(DDRbiasLayer_$layer_index = startOfDDR, biasLayer_$layer_index, sizeof(biasLayer_$layer_index)); startOfDDR = (uint8_t*)(((uint32_t)startOfDDR + sizeof(biasLayer_$layer_index) + 0x1000) & 0xFFFFF000);\n";

                print $hfile_hnd "uint16_t biasLayer_$layer_index"."[] = {\n\t";
                for($och = 0; $och < $layer{"tapDimZ"}; $och++)
                {
                    print $outBlob_hnd $bias_data{"o".$och};
                    print $hfile_hnd sprintf "0x%04X, ", unpack("v", $bias_data{"o".$och});
                }
                print $hfile_hnd "\n};\n\n";
            }
            close $hfile_hnd;            
...
sub WriteBasicApiFunctionCalls
{
    my $inputAddr = 0;
    my $outputAddr = 0;
    my $sourceCode;
    ...
    my @layer_types = ("convolution", "max_pooling", "average pooling", "softmax", "fully connected", "none", "relu", "reluX", "depth convolution", "bias", "prelu", "LRN");
    my %usedBuffers;
    $sourceCodeGenNetwork .= "void $network_name(void)\n{\n";
    $sourceCodeGenNetwork .= "\tint valid = 0;\n";

    for(my $layer_index = 0; $layer_index < (scalar @layers); $layer_index++)
    {
        my %layer = %{$layers[$layer_index]};       

        $sourceCodeGenNetwork .= "\t//".$layer_types[$layer{"stageType"}]." layer ".$layer{"stageName"}."\n";
        $curLayer = "&cnnLayers[".$layer_index."]";
        
        $inputAddr = "tempDataBuffer_".($Index2OptimizedIndex{$layer{"inputIndex"}} - 2);
        if(1 == $Index2OptimizedIndex{$layer{"inputIndex"}})
        {
            $inputAddr = "inDataBuffer";            
        }

        my $output_offset = ($layer{"OutputOffset"} / 2) * $layer{"outputDimY"} * $layer{"outputDimX"} * 2;
        $outputAddr = "tempDataBuffer_".($Index2OptimizedIndex{$layer{"outputIndex"}} - 2);
        if(2 == $Index2OptimizedIndex{$layer{"outputIndex"}})
        {
            $outputAddr = "outDataBuffer";            
        }
        $usedBuffers{$inputAddr} = 1;
        $usedBuffers{$outputAddr} = 1;
        ...
        if($layer{"stageType"} == 0) #convolution
        {
            $outputAddr = sprintf $outputAddr." + 0x%08X", $output_offset;            
            $sourceCodeGenNetwork .= "\tcnnInitDescriptor($curLayer, MODE_FP16, 0x0A, 0, 0, $prevLayer);\n";            
            $sourceCodeGenNetwork .= "\tcnnSetupInputBrief($curLayer, $inputAddr, ".$layer{"inputDimZ"}.", ".$layer{"inputDimX"}.", ".$layer{"inputDimY"}.");\n";
            $sourceCodeGenNetwork .= "\tcnnSetupOutputBrief($curLayer, $outputAddr, ".$layer{"outputDimZ"}.");\n";
            $sourceCodeGenNetwork .= "\tcnnSetupConvolutionCoefficientsBrief($curLayer, FP16_COEFF, DDRweightsLayer_$layer_index, ".$layer{"radixX"}.", ".$layer{"radixY"}.", ".$layer{"strideX"}.", 0);\n";
            $sourceCodeTempBuffers .= "uint8_t *DDRweightsLayer_$layer_index;\n";
            if($padEn)
            {
                $sourceCodeGenNetwork .= "\tcnnSetupPadding($curLayer, PAD_WITH_ZEROS);\n";
            }
            if($reluEn)
            {
                $sourceCodeGenNetwork .= "\tcnnSetupReLu($curLayer, 0, 0, 1);\n";
            }
            if($biasEn)
            {
                $sourceCodeGenNetwork .= "\tcnnSetupBias($curLayer, DDRbiasLayer_$layer_index);\n";
                $sourceCodeTempBuffers .= "uint8_t *DDRbiasLayer_$layer_index;\n";
            }
            $sourceCodeGenNetwork .= "\tvalid = cnnFinalizeLayer($curLayer);\n";
            $sourceCodeGenNetwork .= "\tif(0 != valid) exit(100 + $layer_index); //Invalid configuration\n";
            $sourceCodeGenNetwork .= "\n";
            $prevLayer = $curLayer;             
        }
        elsif(($layer{"stageType"}) == 1 || ($layer{"stageType"} == 2)) #pooling (max or average)
        {
            $padType = "PAD_WITH_ZEROS +";
            my $poolType = $layer{"stageType"} - 1; #0-max; 1-average
            if ($poolType == 0)
            {
                $poolType = "POOL_MAX"; 
            ...
            $sourceCodeGenNetwork .= "\tcnnSetupInputBrief($curLayer, $inputAddr, ".$layer{"inputDimZ"}.", ".$layer{"inputDimX"}.", ".$layer{"inputDimY"}.");\n";
            $sourceCodeGenNetwork .= "\tcnnSetupOutputBrief($curLayer, $outputAddr, ".$layer{"outputDimZ"}.");\n";
            $sourceCodeGenNetwork .= "\tcnnSetupPoolingLayer($curLayer, $poolType, ".$layer{"radixX"}.", ".$layer{"radixY"}.", ".$layer{"strideX"}.");\n";
            
            if($padEn)
            {
                $sourceCodeGenNetwork .= "\tcnnSetupPadding($curLayer, $padType);\n";
            }
            if($reluEn)
            {
                $sourceCodeGenNetwork .= "\tcnnSetupReLu($curLayer, 0, 0, 1);\n";
        ...
        elsif(($layer{"stageType"}) == 4) #fully connected
        {
            ...
            while($remVectors)
            {                
                $curLayer = "&cnnLayers[".($layer_index + $lindex)."]";
                my $vectorsThisTime = $remVectors;
                if($vectorsThisTime > 256)
                {
                    $vectorsThisTime = 256;
                ...
                $remVectors -= $vectorsThisTime;           
                my $tempoutputAddr = sprintf $outputAddr." + 0x%08X", ($output_offset + $lindex * 256 * 2);
                my $weightsOffset = sprintf "0x%08X", ($lindex * $inputWidth * 256 * 2);
                my $biasOffset = sprintf "0x%08X", ($lindex * 256 * 2);
                $sourceCodeGenNetwork .= "\tcnnInitDescriptor($curLayer, MODE_FP16, 0x0A, 0, 0, $prevLayer);\n";                        
                $sourceCodeGenNetwork .= "\tcnnSetupInputFullyConnectedLayerBrief($curLayer, $inputAddr, $inputWidth);\n";            
                $sourceCodeGenNetwork .= "\tcnnSetupOutputFullyConnectedLayerBrief($curLayer, $tempoutputAddr);\n";
                $sourceCodeGenNetwork .= "\tcnnSetupVectorsFullyConnectedLayerBrief($curLayer, FP16_COEFF, DDRweightsLayer_$layer_index + $weightsOffset, $vectorsThisTime);\n";                
                if($reluEn)
                {
                    $sourceCodeGenNetwork .= "\tcnnSetupReLu($curLayer, 0, 0, 1);\n";
                }
                if($biasEn)
                {
                    $sourceCodeGenNetwork .= "\tcnnSetupBias($curLayer, DDRbiasLayer_$layer_index + $biasOffset);\n";
                }
                $sourceCodeGenNetwork .= "\tvalid = cnnFinalizeLayer($curLayer);\n";
                $sourceCodeGenNetwork .= "\tif(0 != valid) exit(100 + $layer_index + $lindex); //Invalid configuration\n";
                $sourceCodeGenNetwork .= "\n";
                $prevLayer = $curLayer;
                $lindex++;
            }
    ...
    $sourceCodeGenNetwork .= "\tcnnStart(CNN_BLOCK0, CNN_LANE0, cnnLayers);\n";
	$sourceCodeGenNetwork .= "\tcnnWaitForBlock(CNN_BLOCK0);\n";
    $sourceCodeGenNetwork .= "}\n";
    foreach my $tempbuf (sort keys %usedBuffers)
    {
        $sourceCodeTempBuffers .= "uint8_t *$tempbuf;\n";
        $sourceCodeCopyFunction .= "\t$tempbuf = startOfDDR; startOfDDR = startOfDDR + (10 * 1024 * 1024);\n";
    }
    $sourceCodeCopyFunction .= "\tcnnLayers = (cnnLayer*)startOfDDR; startOfDDR = startOfDDR + (10 * 1024 * 1024);\n";
}

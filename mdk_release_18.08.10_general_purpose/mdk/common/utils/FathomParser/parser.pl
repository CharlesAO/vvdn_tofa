...
sub WriteCNNStruct2Blob
{
    ...
    for(my $layer_index = 0; $layer_index < (scalar @layers); $layer_index++)
    {
        ...
        if($layer{"stageType"} == 0) #convolution
        {
            ...
            my $coeff_LPB = $layer{"inputDimZ"} * $layer{"radixX"} * $layer{"radixY"} - 1;
            my $css = $layer{"radixX"} * $layer{"radixY"} - 1;
            #Create convolution layer structure
            my $layer_struct .= push_bits([11,$rsvd], [1, 0], [4,$rsvd], [4, 0], [4, 0x0A], [2,$rsvd], [3, 0], [3, 0], [32, $link_addr]);
            ...
            $layer_struct .= push_bits([1, $padEn], [4, $padType], [19,$rsvd], [4, $layer{"radixX"}-1], [4, $layer{"radixY"}-1], [12,$rsvd], [4, $layer{"strideX"}-1], [5,$rsvd], [11, $layer{"inputDimZ"}-1]);
            ...
            $layer_struct .= push_bits([4,$rsvd], [12, $layer{"outputDimX"}], [8, $css], [8, $coeff_LPB], [23,$rsvd], [9, 0]);

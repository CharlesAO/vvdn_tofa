...
SECTIONS {
  ...
  ASSERT( (__STACK_SIZE > __MIN_STACK_SIZE) && (__STACK_SIZE < 0x80000000) , "Leon Code + data too big. Stack space left is smaller than the required minimum (3.5 KB)!")
  
  __INIT_STACK_SIZE = ABSOLUTE(__sys_bss_start - __init_text_end);
  ASSERT( (__INIT_STACK_SIZE > __MIN_INIT_STACK_SIZE) && (__INIT_STACK_SIZE < 0x80000000) , "Stack available to the init code is smaller than the specified minimum (which by default is 1 KB)!")

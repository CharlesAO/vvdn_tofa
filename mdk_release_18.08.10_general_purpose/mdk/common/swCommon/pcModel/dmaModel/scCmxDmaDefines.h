...
typedef struct ScCmxDmaTransactionList {
    ...
    uint32_t sw_id;
    uint32_t current_id;
    uint32_t completed; // PC model only
} ScCmxDmaTransaction;  // dmaTransactionLists must be aligned to 64 bit
                                                       // boundaries (This is a requirement of the
                                                       // hardware DMA engine)

typedef struct ScCmxDmaTransactionHnd {
    ScCmxDmaTransaction transaction; // PC model only: keep transaction info accessible for easy PC model implementation

...
void scDmaSetup(swcDmaTask_t taskNum, void *src, void *dst, unsigned int numBytes)
{
    ...
    dmaTasks[task].p_dst        = (u8*)dst;
    dmaTasks[task].p_src        = (u8*)src;
...
/// @param[in] - dst - destination for task
/// @param[in] - numBytes - length in bytes
/// @param[in] - width - line width of the copied block
/// @param[in] - stride - image step
/// @return    - void
///
void scDmaSetupStrideDst(unsigned int task, u8* src, u8* dst, unsigned int numBytes, unsigned int width, unsigned int stride)
{
    ...
    dmaTasks[taskNum].p_dst        = dst;
    dmaTasks[taskNum].p_src        = src;
...
void scDmaStart(unsigned int taskNum)
{
    ...
    {
		p_dst_adr=dmaTasks[i].p_dst; //set p_dst_adr with the dmaTasks[i].p_dst address, in order p_dst to remain intact for scDmaWaitFinished
		p_src_adr=dmaTasks[i].p_src; //the same with p_src_adr
        ...
        {
			if (dmaTasks[i].strideSrc != 0 && dmaTasks[i].strideDst != 0)
				{
					for(u32 j=0;j<dmaTasks[i].line_width;j++)
					{
						memset(p_dst_adr, 0xFF, dmaTasks[i].line_width);
						p_src_adr+=dmaTasks[i].line_width;
						p_src_adr+=dmaTasks[i].strideSrc;
						p_dst_adr+=dmaTasks[i].line_width;
						p_dst_adr+=dmaTasks[i].strideDst;
					}
				}
			else
			{
				...
				if (dmaTasks[i].strideDst != 0 && dmaTasks[i].strideSrc == 0)
				{
					for(u32 k=0;k<dmaTasks[i].line_width;k++)
					{
						memset(p_dst_adr, 0xFF, dmaTasks[i].line_width);
						p_dst_adr+=dmaTasks[i].line_width;
						p_dst_adr+=dmaTasks[i].strideDst;
		...
		if (!dmaTasks[i].completed)
        {
            ...
            else
            {
				if (dmaTasks[i].strideSrc != 0 && dmaTasks[i].strideDst != 0)
				{
					for(u32 j=0;j<dmaTasks[i].line_width;j++)
					{
						memcpy(dmaTasks[i].p_dst, dmaTasks[i].p_src, dmaTasks[i].line_width);
						dmaTasks[i].p_src+=dmaTasks[i].line_width;
						dmaTasks[i].p_src+=dmaTasks[i].strideSrc;
						dmaTasks[i].p_dst+=dmaTasks[i].line_width;
						dmaTasks[i].p_dst+=dmaTasks[i].strideDst;
					}
					dmaTasks[i].completed  = 1;
				}
				else

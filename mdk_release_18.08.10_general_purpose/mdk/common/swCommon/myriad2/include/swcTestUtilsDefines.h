...
///
/// @defgroup swcTestUtilsDefines Test Utils Defines
/// @ingroup  TestUtils
/// @{
...
typedef struct
{
     ...
     unsigned long long perfCounterTimer;   ///< counts the total execution of
                                            /// the program
     u32 countShCodeRun;    ///< counts how many times the shave code was executed
     u32 stallsTypes;       ///< enables specific stalls from a given list to be counted
     tyTimeStamp executionTimer;    ///< assignes its value to perfCounterTimer
                                    /// in order to display total execution
                                    /// (in cycles, [us] and [ms])
}performanceStruct;

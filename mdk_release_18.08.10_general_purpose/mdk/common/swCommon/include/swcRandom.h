...
/// @brief Reset the base seed of the PRNG.
/// @note This is equivalent to srand() from standard C.
/// @param[in] initValue - 64 bit initial value
void swcRandInit(u64 initValue);

/// @brief Get next 64 bit random value in sequence defined by
/// the global seed which was set using swcRandInit().
/// @note This is equivalent to rand() from standard C.
/// @return    64 bit pseudo random value between [0, RAND_MAX]
u64 swcRandGetRandValue(void);

/// @brief Get next 64 bit random value in sequence defined by seed.
///
...
/// be the same.
/// @note This is equivalent to rand_r() from standard C.

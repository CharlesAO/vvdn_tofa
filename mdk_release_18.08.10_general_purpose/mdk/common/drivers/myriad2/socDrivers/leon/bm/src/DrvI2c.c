...
#define SETUP_HPART_MASK 0xffff0000
...
u32 DrvIicRd8ba(u32 i2cBase, u32 adr) {
    ...
    if((GET_REG_WORD_VAL(i2cBase + IIC_INTR_STAT) & IIC_IRQ_TX_ABRT)) {
        txAbrtSrc = GET_REG_WORD_VAL(i2cBase + IIC_TX_ABRT_SOURCE);
        clr = GET_REG_WORD_VAL(i2cBase + IIC_CLR_INTR);
        return SETUP_HPART_MASK | txAbrtSrc;
    }
    ...
    while(size) {
        ...
        for (i = 0 ; i < n - 1 ; ++i)
        {
            while (!(GET_REG_WORD_VAL(i2cBase + IIC_STATUS) & IIC_STATUS_TX_FIFO_NOT_FULL))        /* Wait while FIFO full */
                ;
            SET_REG_WORD(i2cBase + IIC_DATA_CMD, swcLeonGetByte(data + i + addr16 - init_addr16, NATIVE_POINTER_TYPE) & 0xFF);
        }
        // Send stop bit
        while (!(GET_REG_WORD_VAL(i2cBase + IIC_STATUS) & IIC_STATUS_TX_FIFO_NOT_FULL))        /* Wait while FIFO full */
            ;
        SET_REG_WORD(i2cBase + IIC_DATA_CMD, (swcLeonGetByte(data + n - 1 + addr16 - init_addr16, NATIVE_POINTER_TYPE) & 0xFF) | IIC_DATA_CMD_STOP);
...
void DrvIicEepromRd(u32 i2cBase, u32 addr16, u8 *data, u32 size)
{
    ...
    u32 pendingWrite, icBit;

    do {

        SET_REG_WORD(i2cBase + IIC_DATA_CMD, (addr16 >> 8) & 0xFF);
        SET_REG_WORD(i2cBase + IIC_DATA_CMD, (addr16 & 0xFF) | (IIC_DATA_CMD_STOP));
    ...
    while(size) {
        ...
        for(i =0; i < n; ++i) {
            while (!(GET_REG_WORD_VAL(i2cBase + IIC_STATUS) & IIC_STATUS_RX_FIFO_NOT_EMPTY))       /* Wait if the RX_FIFO becomes empty */
                ;
            swcLeonSetByte(data, NATIVE_POINTER_TYPE, GET_REG_WORD_VAL(i2cBase + IIC_DATA_CMD));
            data++;
        }
        size -= n;
    }
...
int getIicHCNTcc(u32 timeNs)
{
    int rv = 0, spklen = 0;
    // ic_clk in ns. I2C it's on the slow AHB, so we get half of the sys_clk
    float ic_clk = 2.0 * 1000000.0 / DrvCprGetSysClockKhz();
    // SPKLEN is different in HS mode than in SS and FS
    if(timeNs == I2C_HS_HCNT)
        spklen = I2C_SPKLEN_HS;
    else
        spklen = I2C_SPKLEN_SS_FS;


    rv = (int) ((float) (((float) (timeNs - I2C_SCL_FALL_TIME_NS)) / ic_clk + 0.5)
                - 7 - spklen);
    if (rv <= 0)
    {
    	DPRINTF1("getIicHCNTcc() FAILED to calculate SCL HCNT: Increase the system clock frequency\n");
    }
    return rv;
}

// SCL_L_TINME = (LCNT + 1)*IC_CLK - SCL_FallTime + SCL_RiseTime
int getIicLCNTcc(u32 timeNs)
{
    int rv = 0;
    // ic_clk in ns. I2C it's on the slow AHB, so we get half of the sys_clk
    float ic_clk = 2.0 * 1000000.0 / DrvCprGetSysClockKhz(); // ic_clk in ns
    rv = (int) ((float) (((float) (timeNs + I2C_SCL_FALL_TIME_NS - I2C_SCL_FALL_TIME_NS)) / ic_clk + 0.5)
                - 1);
    if (rv <= 0)
    {
    	DPRINTF1("getIicLCNTcc() FAILED to calculate SCL HCNT: Increase the system clock frequency\n");
    }
    return rv;
}
...
void DrvIicMasterInit(u32 i2cBase, u32 addr, u8 thl, u32 speed)
{
    ...
    if (speed == 0x3)
    {
        ...
        {
        	DPRINTF1("DrvIicMasterInit() FAILED\n");
        }
        else
        {
            DrvIicSetSpeedHigh(i2cBase, (u32) scl_hcnt, (u32) scl_lcnt);
        }
    }
    else if (speed == 0x2)
    {
        ...
        {
            DrvIicSetSpeedFast(i2cBase, (u32) scl_hcnt, (u32) scl_lcnt);
        }
    }
    else  // 0x1
    {
        ...
        {
            DrvIicSetSpeedSlow(i2cBase, (u32) scl_hcnt, (u32) scl_lcnt);
        }
    }
...
void DrvIicSlaveInit(u32 i2cBase, u32 addr, u8 thl, u32 speed)
{
    ...
    if (speed == 0x3)
    {
        ...
        if ((scl_hcnt <= 0) || (scl_lcnt <= 0))
        {
        	DPRINTF1("DrvIicSlaveInit() FAILED\n");
        }
        else
        {
            DrvIicSetSpeedHigh(i2cBase, (u32) scl_hcnt, (u32) scl_lcnt);
...
*/
    u32 icBit;
    //DPRINTF1("TX_ABRT Interrupt asserted\n");
    ...
    if ((GET_REG_WORD_VAL(i2cBase + IIC_RAW_INTR_STAT) & IIC_IRQ_TX_ABRT) != 0) {
        //DPRINTF1("The source of TX_ABRT Interrupt could not be cleared\n");
        //DPRINTF1("Exiting Test ...\n");
        exit(4);
    }
}
...
static void i2cRecovery(u32 i2cBase)
{
    ...
    SET_REG_WORD (CPR_BLK_RST0_ADR, (data & resetBit));
...
/* MAIN GOAL OF THE FUNCTION :
    ...
    if(((GET_REG_WORD_VAL(i2cBase + IIC_RAW_INTR_STAT)) & IIC_IRQ_RD_REQ))                         /* Check to see if RD_REQ Int cleared */
        exit(5); // Error state
        //DPRINTF1("ERROR: RD_REQ Int NOT CLEARED cleared\n");
...
/* MAIN GOAL OF THE FUNCTION :
    ...
    */
    ...
    DPRINTF1("Data was transfered successfuly to the slave\n");
}
 ...
 */
u32 DrvIicProgramDev(u32 i2cBase, u32 sclFreqKhz, u32 i2cAddr,
                     u32 *setupArray16[2],
                     u32 items)
{
    u32 i, speed = 1, scl_hcnt, scl_lcnt;
    ...
    if (sclFreqKhz > 99)
    {
        ...
        if ((scl_hcnt <= 0) || (scl_lcnt <= 0))
        {
        	DPRINTF1("DrvIicProgramDev() FAILED\n");
        }
        else
        {
            DrvIicSetSpeedFast(i2cBase, (u32) scl_hcnt, (u32) scl_lcnt);
    ...
    for (i = 0; i < items; ++i)
    {
        if (DrvIicWr16(i2cBase, setupArray16[i][0], setupArray16[i][1]))
        {  // if it failed once
 ...
 *       the i2c addr was wrong or another master was doing transactions on the bus
 */
u32 DrvIicProgramDev2(u32 sclFreqKhz, u32 i2cAddr, u32 *setupArray16[2],
                      u32 items)
{
    u32 i, speed = 1, txAbrtSrc, scl_hcnt, scl_lcnt;
    volatile u32 clr = 0; UNUSED(clr);
    ...
    if (sclFreqKhz > 99) // Fast mode
    {
        ...
        if ((scl_hcnt <= 0) || (scl_lcnt <= 0))
        {
        	DPRINTF1("DrvIicProgramDev2() FAILED\n");
        }
        else
        {
            DrvIicSetSpeedFast(IIC1_BASE_ADR, (u32) scl_hcnt, (u32) scl_lcnt);
            DrvIicSetSpeedFast(IIC2_BASE_ADR, (u32) scl_hcnt, (u32) scl_lcnt);
        }
        speed++;
    }
    else
    {
        ...
        {
            DrvIicSetSpeedSlow(IIC1_BASE_ADR, (u32) scl_hcnt, (u32) scl_lcnt);
            DrvIicSetSpeedSlow(IIC2_BASE_ADR, (u32) scl_hcnt, (u32) scl_lcnt);
        }
    }
    ...
    for (i = 0; i < items; ++i)
    {
        SET_REG_WORD(IIC1_BASE_ADR + IIC_DATA_CMD,
                     (setupArray16[i][0] >> 8) & 0xFF);
        ...
        SET_REG_WORD(IIC2_BASE_ADR + IIC_DATA_CMD,
                     (setupArray16[i][0] >> 8) & 0xFF);
...
void DrvIicBitbangInit( u32 sdaPin, u32 sclPin, u32 tar,  u32 hCntl, u32 lCntl ) {
    ...
    DrvGpioSetMode( gSdaPin, D_GPIO_MODE_7 | D_GPIO_DIR_IN );
    DrvGpioSetMode( gSclPin, D_GPIO_MODE_7 | D_GPIO_DIR_IN );
...
#define setIicPinHi( pinr ) DrvGpioSetMode( pinr, D_GPIO_MODE_7 | D_GPIO_DIR_IN )
#define setIicPinLo( pinr ) DrvGpioSetMode( pinr, D_GPIO_MODE_7 )

static inline void drvIicBitbangWr( u32 byte ) {
    int i;

    for(i=0; i<8; i++,byte <<= 1 ) {
        DrvTimerSleepTicks( delay1 ); // wait fixed_delay
        DrvGpioSetMode( gSdaPin, D_GPIO_MODE_7 | ((byte & 0x80 )<< 4 )); //set data [hi]
        DrvTimerSleepTicks( delay2 );// wait lcntl-hcntl-fixed_delay
        setIicPinHi( gSclPin ); //set clk hi
        DrvTimerSleepTicks( delay3 ); //wait hcnt
        setIicPinLo( gSclPin ); //set clk lo
        //DrvTimerSleepTicks( delay1 ); // wait fixed_delay
        //set data [low]
    }
    DrvTimerSleepTicks( delay2 ); // wait fixed_delay
    setIicPinHi( gSdaPin ); // free sda pin for ACK
}

static inline u32 drvIicBitbangRd(void) {
    volatile u32 data;
    int i;
...
static inline u32 drvIicBitbangCkAck(void) {
    u32 dataLine;
    //toggle SCL one
    DrvTimerSleepTicks( delay2 );// wait lcntl-hcntl-fixed_delay
    setIicPinHi( gSclPin ); //set clk hi
...
static inline void drvIicBitbangS(void) {
    // SCL=1, SDA=0
    setIicPinHi( gSclPin );  // Clk must be high before doing a start
    DrvTimerSleepTicks( delay3 ); // insert one CLK delay here
...
static inline void drvIicBitbangP(void) {
...
static inline void drvIicBitbangSendAck(void) {
...
NACK_RETURN:
    ...
    return SETUP_HPART_MASK | state; // error is 32 bit while data is 8 bit
}

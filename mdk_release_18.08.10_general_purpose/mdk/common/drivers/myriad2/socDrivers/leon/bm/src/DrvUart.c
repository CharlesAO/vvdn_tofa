...
static void drvUartRegConfig(tDrvUartCfg * uartCfg, u32 sysSpeedHz, u32 divider)
{
    UNUSED(sysSpeedHz); // hush the compiler warning.
    u32 lcr_register = 0;

    SET_REG_WORD(UART_MCR_ADR, D_UART_MCR_LB); // Set loopback mode to ensure that no character is received during initialization
    //Write to DLH and DLL to set up the divisor
    ...
    SET_REG_WORD(UART_LCR_ADR, 0);                  //set latch acces bit to 0
    ...
    case (UartDataBits5):
        lcr_register |= D_UART_LCR_DLS5;
        break;
    case (UartDataBits6):
        lcr_register |= D_UART_LCR_DLS6;
        break;
    case (UartDataBits7):
        lcr_register |= D_UART_LCR_DLS7;
        break;
    case (UartDataBits8):
    default:
        lcr_register |= D_UART_LCR_DLS8;
    }

    switch (uartCfg->StopBits) {
    case (UartStopBit1):
        lcr_register |= D_UART_LCR_STOP1;
        break;
    case (UartStopBit2):
    default:
        lcr_register |= D_UART_LCR_STOP2;
    }

    switch (uartCfg->Parity) {
    case (UartParityOdd):
        lcr_register |= D_UART_LCR_PEN;
        break;
    case (UartParityEven):
        lcr_register |= D_UART_LCR_EPS | D_UART_LCR_PEN;
        break;
    case (UartParityNone):
    default:
    ...
    // 5. If FIFO_MODE != NONE (FIFO mode enabled), write to FCR to enable FIFOs and set Transmit FIFO threshold level
    SET_REG_WORD(UART_FCR_ADR, D_UART_FCR_RCVR_HALF  |// rx interrupt
            D_UART_FCR_TET_HALF |// tx interrupt
            D_UART_FCR_XFIFOR |// XMIT FIFO Reset
            D_UART_FCR_RFIFOR |// RCVR FIFO Reset
            D_UART_FCR_FIFOE// enable FIFO
            );

    // 6. Write to IER to enable required interrupts
    ...
    // 7. Write to Modem Control Register (MCR) to program SIR mode, auto flow, loopback, modem control outputs
...
{
    u32 parity;
    u32 __attribute__((unused)) divider; // avoid 'unused variable' error from scan build
    if (sysSpeedHz == 0)
        return D_UART_INIT_FUNC_STATUS_DIVIDER0;
    if (uartCfg->BaudRate == 0)
        return D_UART_INIT_FUNC_STATUS_BAUDRATE0;

    divider = (sysSpeedHz << 1) / (uartCfg->BaudRate * BIT_TIME);
    parity = divider & 1;
    divider = (divider >> 1) + parity;

    if (divider == 0)
    {
        divider = 1; // If divider is 0 UART is not transmittig.
        return D_UART_INIT_FUNC_STATUS_INVALID_BAUD;
    }

    drvUartRegConfig(uartCfg, sysSpeedHz, divider);

    return D_UART_INIT_FUNC_STATUS_OK;
}

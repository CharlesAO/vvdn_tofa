...
typedef void * (*SIPP_PAL_PFNTHREAD)(void **);
typedef void * (*SIPP_RTEMS_PFNTHREAD)(void *);
...
typedef struct
{
    ...
    uint32_t  stackSize;
    uint8_t   priority;
    pthread_t rtemsThread;

} SIPP_PAL_THREAD, * SIPP_PAL_THREAD_ID;

typedef struct
{
    uint16_t elementsInQ;
    uint16_t maxQElements;
    uint16_t qFlags;
    mqd_t    rtemsQuId;                 // Used at creation time
    mqd_t    rtemsRWQuId;

} SIPP_PAL_QU, * SIPP_PAL_QU_ID;

typedef u32  SIPP_PAL_QU_MSG;
...
#define SIPP_QU_FLAG_STRUCT_FULL  0x1

...
#include <filters/bitwiseOrMask/bitwiseOrMask.h>

/// per-element bit-wise logical conjunction(OR) for two arrays if mask == 1
...
#ifdef SIPP_USE_MVCV
void mvcvBitwiseOrMask_asm(UInt8** src1Addr, UInt8** src2Addr, UInt8** destAddr, UInt8** maskAddr, UInt32 width);
#else
void bitwiseOrMask        (UInt8** src1Addr, UInt8** src2Addr, UInt8** destAddr, UInt8** maskAddr, UInt32 width)
{
    UInt32 i;
    UInt8* src1;
    UInt8* src2;
...
void svuBitwiseOrMask(SippFilter *fptr)
{
    UInt32 i;
    UInt8 *input1;
    UInt8 *input2;
    ...
    #ifdef SIPP_USE_MVCV
        mvcvBitwiseOrMask_asm(&input1, &input2, &output, &mask, fptr->sliceWidth);
    #else
        bitwiseOrMask(&input1, &input2, &output, &mask, fptr->sliceWidth);
    #endif
    }
}

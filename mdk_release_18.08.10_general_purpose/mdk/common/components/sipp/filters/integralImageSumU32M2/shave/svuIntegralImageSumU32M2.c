...
#include <filters/integralImageSumU32M2/integralImageSumU32M2.h>
...
#ifdef SIPP_USE_MVCV
void mvcvIntegralimage_sum_u32_M2_asm(UInt32* out, UInt8* in, UInt32 runNr, UInt32** prevPointer , UInt32 width);
#else
void integralimage_sum_u32M2(UInt32* out, UInt8* in, UInt32 runNr, UInt32** prevPointer , UInt32 width)
{
    if (0 == runNr)
    {
        UInt32 x;
...
void svuIntegralImageSumU32M2(SippFilter *fptr)
{
    UInt32 *outLn;
    UInt8  *inLn;
    UInt32 pl;
    ...
    #ifdef SIPP_USE_MVCV
            mvcvIntegralimage_sum_u32_M2_asm(outLn, inLn, runNo, &previsionOutLnPointer[pl], fptr->sliceWidth);
    #else
            integralimage_sum_u32M2(outLn, inLn, runNo, &previsionOutLnPointer[pl], fptr->sliceWidth);
    #endif
        }
    }
}

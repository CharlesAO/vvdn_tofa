...
#include <filters/bitwiseAnd/bitwiseAnd.h>
...
#ifdef SIPP_USE_MVCV
void mvcvBitwiseAnd_asm(UInt8** src1Addr, UInt8** src2Addr, UInt8** destAddr, UInt32 width);
#else
void bitwiseAnd        (UInt8** src1Addr, UInt8** src2Addr, UInt8** destAddr, UInt32 width)
{
    UInt32 i;
    UInt8* src1;
    UInt8* src2;
...
void svuBitwiseAnd(SippFilter *fptr)
{
    UInt8 *input1;
    UInt8 *input2;
    UInt8 *output;
    ...
    #ifdef SIPP_USE_MVCV
        mvcvBitwiseAnd_asm(&input1, &input2, &output, fptr->sliceWidth);
    #else
        bitwiseAnd(&input1, &input2, &output, fptr->sliceWidth);
    #endif
    }
}

...
#include <filters/integralImageSqSumF32M2/integralImageSqSumF32M2.h>
...
#ifdef SIPP_USE_MVCV
void mvcvIntegralimage_sqsum_f32_M2_asm(float* out, UInt8* in, UInt32 runNr, float** prevPointer, UInt32 width);
#else
void integralimage_sqsum_f32_M2        (float* out, UInt8* in, UInt32 runNr, float** prevPointer, UInt32 width)
{
    if (0 == runNr)
    {
        unsigned int x;
...
void svuIntegralImageSqSumF32M2(SippFilter *fptr)
{
    float *outLn;
    UInt8  *inLn;
    UInt32 pl;
    ...
    #ifdef SIPP_USE_MVCV
            mvcvIntegralimage_sqsum_f32_M2_asm(outLn, inLn, runNo, &previsionOutLnPointer[pl], fptr->sliceWidth);
    #else
            integralimage_sqsum_f32_M2(outLn, inLn, runNo, &previsionOutLnPointer[pl], fptr->sliceWidth);
    #endif
        }
    }
}

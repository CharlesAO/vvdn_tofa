...
#include <filters/padBayer5Frame/padBayer5Frame.h>

//##########################################################################################
void padBayer5Reference(uint16_t *inLine[13],
                        uint16_t *outLine,
                        uint32_t exeNo,
...
void svuPadBayer5Frame(SippFilter *fptr)
{
    uint16_t *output;
    uint16_t *inLine[13];
    ...
    for (iterNum = 0; iterNum < numLinesThisIter; iterNum++, runNo++)
    {
        ...
        output = (uint16_t*)getPlaneIoPtrs(fptr, 0/*parent*/, iterNum, 0/*plane*/, inLine);

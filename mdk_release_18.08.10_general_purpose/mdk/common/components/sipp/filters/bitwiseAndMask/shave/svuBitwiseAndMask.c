...
#include <filters/bitwiseAndMask/bitwiseAndMask.h>

/// per-element bit-wise logical conjunction(AND) for two arrays if mask == 1
...
#ifdef SIPP_USE_MVCV
void mvcvBitwiseAndMask_asm(UInt8** src1Addr, UInt8** src2Addr, UInt8** destAddr, UInt8** maskAddr, UInt32 width);
#else
void bitwiseAndMask        (UInt8** src1Addr, UInt8** src2Addr, UInt8** destAddr, UInt8** maskAddr, UInt32 width)
{
    UInt32 i;
    UInt8* src1;
    UInt8* src2;
...
void svubitwiseAndMask(SippFilter *fptr)
{
    UInt32 i;
    UInt8 *input1;
    UInt8 *input2;
    ...
    #ifdef SIPP_USE_MVCV
        mvcvBitwiseAndMask_asm(&input1, &input2, &output, &mask, fptr->sliceWidth);
    #else
        bitwiseAndMask(&input1, &input2, &output, &mask, fptr->sliceWidth);
    #endif
    }
}

...
#ifdef SIPP_USE_MVCV
void mvcvArithmeticSub_asm      (UInt8** src1, UInt8** src2, UInt8** dst, UInt32 width);
#else
void arithmeticSubImplementation(UInt8** src1, UInt8** src2, UInt8** dst, UInt32 width)
{
    int i;
    float sub;
    for(i=0;i<width;i++)
...
void svuArithmeticSub(SippFilter *fptr)
{
    UInt8 *input1[1];
    UInt8 *input2[1];
    UInt8 *output;
    ...
    #ifdef SIPP_USE_MVCV
        mvcvArithmeticSub_asm(input1, input2, &output, fptr->sliceWidth);
    #else
        arithmeticSubImplementation(input1, input2, &output, fptr->sliceWidth);
    #endif
    }
}

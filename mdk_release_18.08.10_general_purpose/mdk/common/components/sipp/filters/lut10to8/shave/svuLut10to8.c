...
#ifdef SIPP_USE_MVCV
void mvcvLUT10to8_asm(UInt16** src, UInt8** dest, const UInt8* lut, UInt32 width, UInt32 height);
#else
...
void svuLut10to8(SippFilter *fptr)
{
    UInt8 *output;
    UInt16 *iline[1];
    ...
    #ifdef SIPP_USE_MVCV
        mvcvLUT10to8_asm(iline, &output, param->lutValue, fptr->sliceWidth, 1);
    #else

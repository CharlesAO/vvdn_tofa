...
#include <filters/censusMatching65/censusMatching65.h>
...
#ifdef SIPP_USE_MVCV
void mvcvCensusMatching65_asm(UInt32 *in1, UInt32 *in2, UInt8 *out, UInt32 width);
#else
void mvcvCensusMatching65    (UInt32 *in1, UInt32 *in2, UInt8 *out, UInt32 width)
{
    UInt32 resultXOR, disparities = 65;
    UInt8 bitCount;
...
void svuCensusMatching65(SippFilter *fptr)
{
    UInt32 *input1;
    UInt32 *input2;
    UInt8 *output;
    ...
    #ifdef SIPP_USE_MVCV
        mvcvCensusMatching65_asm(input1, &input2[64], output, parSliceWidth);
    #else
        mvcvCensusMatching65(input1, &input2[64], output, parSliceWidth);
    #endif

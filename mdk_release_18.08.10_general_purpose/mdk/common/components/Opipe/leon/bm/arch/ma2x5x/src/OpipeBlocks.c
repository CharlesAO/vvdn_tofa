...
void cfgColComb(Opipe *pipe)
{
 ...
 //Set 3D LUT
  #define LUT3D_BYPASS (pipe->cfg[SIPP_CC_ID] & (1 << 3))
   if((cfg->lut3D) && !LUT3D_BYPASS)
   {
      SET_REG_WORD(I_BASE(SIPP_CC_3DLUT_ID), (uint32_t)cfg->lut3D); // Set 3D LUT if available
   }
 ...
 //Filter specific registers
   ...
   SET_REG_WORD(SIPP_CC_KRGB1_ADR  ,  cfg->kb | (pipe->pChrGenCfg->epsilon * 16) << 16); //kb(4.8), eps

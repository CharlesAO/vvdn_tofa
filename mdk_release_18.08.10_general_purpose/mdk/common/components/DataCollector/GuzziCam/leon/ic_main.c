...
/// Debug flag for triggering still capture (no-zsl version)
uint32_t dbg_dostill __attribute__((section(".cmx_direct.data"))) = 0;
// 4: Static Local Data
// ----------------------------------------------------------------------------
...
//                    ev.EvInfo.ts+ timeDelay, (ts-(ev.EvInfo.ts+ timeDelay)), ev.EvInfo.seqNr);
        ...
        case IC_EVENT_TYPE_ERROR:
            ...
            if(3 == ev.EvInfo.seqNr) {
                printk("W: Isp cfg mising: %lu \n", ev.EvInfo.instId);
            }
            else {
                printk("W: %lu - %lu - %p\n",ev.EvInfo.seqNr, ev.EvInfo.instId, ((icIspConfig*)ev.EvInfo.userApp01)->userData);
            }
            break;
...
void los_configIsp(void *iconf, int ispIdx) {
    /// Debug test for non-zsl still capture
    if(dbg_dostill && ispIdx == 0)
    {
        /// Set still capture flag in pipectl
        ((icIspConfig *)iconf)->pipeControl |= IC_PIPECTL_STILL_ENABLE;
        dbg_dostill = 0;
    }

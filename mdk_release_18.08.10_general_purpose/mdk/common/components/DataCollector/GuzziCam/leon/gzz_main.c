...
#ifndef METADATA_DEVNAME
#error METADATA_DEVNAME is not defined
#endif
    ...
    )
{
    UNUSED(prv);
    UNUSED(buffer_size);
    //printf("savefile dump.bin %#010x %d", buffer, buffer_size);
...
void send_iq_data (void *data, uint32 size)
{
    ...
    fd = open(METADATA_DEVNAME, O_RDWR);
    if(fd < 0) {
    ...
    )
{
    ...
    if (camera_control_is_active(command->cam.id))
    {
        ...
        if (APP_GUZZI_COMMAND__CAM_START != command->id) {
            mmsdbg(DL_ERROR, "Skipping command %d for Non active camera %lu", command->id, command->cam.id);
            return;
        }
    }
    switch (command->id) {

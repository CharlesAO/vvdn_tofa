...
static void SendUsbI_ProcessingUnitEvent_Sync(
                                    VIDEODEMO_CONTEXT * CONST pVideoDemoCtx,
                                    UPROTO_VIDEO_CONTROL_REQUEST * pRequest);

static void SendUsbI_CameraTerminalEvent_Sync(
                                    VIDEODEMO_CONTEXT * CONST pVideoDemoCtx,
                                    UPROTO_VIDEO_CONTROL_REQUEST * pRequest);

static CALLBACKFN    VideoDemoI_ControlEvent_Sync;
...
void VideoDemo_ControlStatusEvent(void *pClientContext, UPROTO_VIDEO_STATUS    VideoStatusCode)
{
    ...
    USBPUMP_UNREFERENCED_PARAMETER(pVideo);
...
// Name:    VideoDemo_ProcessingControlEvent()
//
// Function:
...
//
// Definition:
//     VOID VideoDemo_ProcessingControlEvent(
//        VOID *        pClientContext,
//        UPROTO_VIDEO_CONTROL_REQUEST *    pRequest
//        );
//
...
//     request to the Control Interface.
//
// Returns:
//     Nothing

void VideoDemo_ControlEvent(void *pClientContext, UPROTO_VIDEO_CONTROL_REQUEST *pRequest)
{
    VIDEODEMO_CONTEXT * CONST pVideoDemoCtx = pClientContext;
...
#ifdef SEND_OUT_USB_SEND_USE_XU
    UPROTO_VIDEO_CONTROL_REQUEST * pRequestVideo = &pVideoDemoCtx->ControlRequest;

    if (pRequestVideo->Extension.bRequestType == UPROTO_VIDEO_CONTROL_REQUEST_XU)
    {
        //UHIL_cpybuf(&pVideoDemoCtx->ControlRequest, pRequest, pRequest->Extension.wLength);
        UHIL_cpybuf(&pVideoDemoCtx->ControlRequest, pRequest, sizeof(pRequest->Hdr) + pRequest->Extension.wLength);
    }
    else
    {
        UHIL_cpybuf(&pVideoDemoCtx->ControlRequest, pRequest, sizeof(*pRequest));
...
static void VideoDemoI_ControlEvent_Sync(void *pClientContext)
{
    VIDEODEMO_CONTEXT * CONST pVideoDemoCtx = pClientContext;
    UPROTO_VIDEO_CONTROL_REQUEST * pRequest = &pVideoDemoCtx->ControlRequest;

    UsbPumpPlatform_MarkCompletionNotBusy(pVideoDemoCtx->pPlatform, &pVideoDemoCtx->SetupCompletion);

    pRequest->Hdr.bAcceptRequest = TRUE;


    switch (pRequest->Hdr.bRequestType)
    {
    case __TMS_UPROTO_VIDEO_CONTROL_REQUEST_VC:
        printf("ERROR: Interface control\n");
        pRequest->Hdr.bAcceptRequest = FALSE;  // Not supported
        break;
    case __TMS_UPROTO_VIDEO_CONTROL_REQUEST_CT:
//        printf("Camera Terminal\n");
        SendUsbI_CameraTerminalEvent_Sync(pVideoDemoCtx, pRequest);
        break;
    case __TMS_UPROTO_VIDEO_CONTROL_REQUEST_SU:
        printf("ERROR: Selector Unit\n");
        pRequest->Hdr.bAcceptRequest = FALSE; // Not supported
        break;
    case __TMS_UPROTO_VIDEO_CONTROL_REQUEST_PU:
//        printf("Processing Unit\n");
        SendUsbI_ProcessingUnitEvent_Sync(pVideoDemoCtx, pRequest);
        break;
    case __TMS_UPROTO_VIDEO_CONTROL_REQUEST_XU:
#ifdef SEND_OUT_USB_SEND_USE_XU
//        printf("Extension Unit\n");
        onEvent(pRequest->Extension.bControlSelector,
                pRequest->Extension.bRequest,
                &pRequest->Extension.wLength,
                &pRequest->Extension.Data[0]);
#else
        printf("ERROR: Extension Unit\n");
        pRequest->Hdr.bAcceptRequest = FALSE; // Not supported
#endif
        break;
    default:
        printf("\nError in Video Control Request: Unexpected %d\n\n", pRequest->Hdr.bRequestType);
        pRequest->Hdr.bAcceptRequest = FALSE;
        break;
    }

    if (pRequest->Hdr.bAcceptRequest == FALSE)
    {
        printf ("Control Unsupported (%d) Type: %d, Selector: %d Request: 0x%02x Length %d\n",
                (int)pRequest->Hdr.bAcceptRequest,
                (int)pRequest->Hdr.bRequestType,
                (int)pRequest->Hdr.bControlSelector,
                (unsigned int)pRequest->Hdr.bRequest,
                (int)pRequest->Hdr.wLength
                );
    }
                                    ...
                                    UPROTO_VIDEO_CONTROL_REQUEST * pRequest)
{
    ...
    {
    case USBVIDEO_bControlSelector_CT_IRIS_ABSOLUTE_CONTROL:
//        printf("IRIS_ABSOLUTE\n");
        switch (pRequest->Hdr.bRequest)
        {
        case USB_bRequest_Video_GET_INFO:
            pRequest->IrisAbs.wIrisAbsolute =
                    CT_IRIS_ABSOLUTE_CONTROL_INFO;
            break;
        case USB_bRequest_Video_GET_MIN:
            pRequest->IrisAbs.wIrisAbsolute =
                    CT_IRIS_ABSOLUTE_CONTROL_MIN;
            break;
        case USB_bRequest_Video_GET_MAX:
            pRequest->IrisAbs.wIrisAbsolute =
                    CT_IRIS_ABSOLUTE_CONTROL_MAX;
            break;
        case USB_bRequest_Video_GET_RES:
            pRequest->IrisAbs.wIrisAbsolute =
                    CT_IRIS_ABSOLUTE_CONTROL_RES;
            break;
        case USB_bRequest_Video_GET_DEF:
            pRequest->IrisAbs.wIrisAbsolute =
                    CT_IRIS_ABSOLUTE_CONTROL_DEF;
            break;
        case USB_bRequest_Video_GET_CUR:
            pRequest->IrisAbs.wIrisAbsolute =
                                    pVideoDemoCtx->CamTerm_Cur.wIrisAbsolute;
//            printf("Get: 0x%x\n", (unsigned int)pVideoDemoCtx->CamTerm_Cur.wIrisAbsolute);
            break;
        case USB_bRequest_Video_SET_CUR:
            // temporarily just accept all setting from host
            pVideoDemoCtx->CamTerm_Cur.wIrisAbsolute =
                                    pRequest->IrisAbs.wIrisAbsolute;
//            printf("Set: 0x%x\n", (unsigned int)pRequest->IrisAbs.wIrisAbsolute);
            in_symbol_process(pRequest->IrisAbs.wIrisAbsolute);
            break;
        default:
            pRequest->Hdr.bAcceptRequest = FALSE;
            break;
...
//        printf("BACKLIGHT_COMPENSATION\n");
 ...
 //       printf("BRIGHTNESS\n");
        ...
        case USB_bRequest_Video_SET_CUR:
#ifdef SEND_OUT_USB_SEND_USE_XU
        	OnUserControlEvent(ISP_USRCTRL_BRIGHTNESS, pRequest->Brightness.wBrightness);
#else
            // temporarily just accept all setting from host
            pVideoDemoCtx->ProcUnitCur.wBrightness =
                    pRequest->Brightness.wBrightness;
...
//        printf("CONTRAST\n");
        ...
        case USB_bRequest_Video_SET_CUR:
#ifdef SEND_OUT_USB_SEND_USE_XU
        	OnUserControlEvent(ISP_USRCTRL_CONTRAST, pRequest->Contrast.wContrast);
#else
...
static void VideoDemoI_StreamInterfaceEvent_Sync(void *pClientContext)
{
    ...
    if (pRequest->Hdr.bAcceptRequest == FALSE)
    {
        printf ("Stream Unsupported (%d) Selector: %d Request: 0x%x Length %d\n",
                (int)pRequest->Hdr.bAcceptRequest,
                (int)pRequest->Hdr.bControlSelector,
                (unsigned int)pRequest->Hdr.bRequest,
                (int)pRequest->Hdr.wLength

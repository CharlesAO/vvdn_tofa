...
static uint32_t sndOutEnableCamera = (uint32_t)(-1);  // bit mask to enable sending frame from type FrameTFormatTypes see IcTypes.h
static uint32_t sndOutEnableFormat = (uint32_t)(-1);  // bit mask to enable sending frame from type FrameTFormatTypes see IcTypes.h
static uint32_t sndOutEnableType   = (uint32_t)(-1);  // bit mask to enable sending frame from type FrameTFormatTypes see IcTypes.h
...
static void SendOut_prepareFrame(SendOutElement_t * tx_task)
{
    ...
    //printf("- %5d, %5d, %5d, %5d, %5d, %5d, %5d\n",outputId, frmType, frame->stride[0], frame->stride[1], frame->height[0], frame->height[1], frame->type);
    sendOutGetBufferHeader()->chunk = 1;
    h->frame_type               = tx_task->frmType;          // FRAME_DATA_TYPE_PREVIEW;
    h->frame_format             = frame->type;      // FRAME_DATA_FORMAT_YUV420;
    h->frame_width              = frame->stride[0];  // PREVIEW_WIDTH;
    h->frame_height             = frame->height[0]; // PREVIEW_HEIGHT;
    h->frame_time_stamp_hi      = (uint32_t)(frame->timestamp[0]>>32);
    h->frame_time_stamp_lo      = (uint32_t)frame->timestamp[0];
    h->frame_proc_time_stamp_hi = (uint32_t)(frame->timestamp[1]>>32);
    h->frame_proc_time_stamp_lo = (uint32_t)frame->timestamp[1];
    h->frame_idx_req_hal        = 0;
    h->frame_idx_req_app        = 0;
    h->frame_idx_mipi_rx        = frame->seqNo;
    h->frame_idx_process        = 0;
    h->header_height            = SEND_OUT_HEADER_HEIGHT; // PREVIEW_HEADER_HEIGHT;
    h->slice_data_type          = 0;
    h->slice_y_offset           = 0;
    ...
    if (h->frame_format == FRAME_T_FORMAT_RAW_10_PACK)
    {
        h->buff_pxl_size_nom        = 5;
        h->buff_pxl_size_denom      = 4;
        h->frame_width = h->frame_width * 4 / 5;
    } else
    if (h->frame_format == FRAME_T_FORMAT_RAW_12_PACK)
    {

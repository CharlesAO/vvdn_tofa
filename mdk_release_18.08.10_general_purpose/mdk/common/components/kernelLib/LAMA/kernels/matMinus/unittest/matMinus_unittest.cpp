...
class matMinusKernelTest : public ::testing::TestWithParam< unsigned int > {
protected:
...
TEST_F(matMinusKernelTest, TestMatrix32x32)
{
    width = 32;
    height = 32;
    ...
    C_c   = mvcvMatMinus(A, B, C_c);
    C_asm = (MAT *)matMinus_asm_test((MAT_test *)A, (MAT_test *)B, (MAT_test *)C_asm);

    RecordProperty("CycleCount", matMinusCycleCount);

	outputChecker.CompareArraysFloat(C_c->base, C_asm->base, width * height + EDGE, 0.0f);
}

TEST_F(matMinusKernelTest, TestMatrix10x10)
{
    width = 10;
    height = 10;
...
TEST_F(matMinusKernelTest, TestMatrixRandom)
{
...
TEST_F(matMinusKernelTest, TestMatrix64x64)
{
    width =  64;
    height = 64;

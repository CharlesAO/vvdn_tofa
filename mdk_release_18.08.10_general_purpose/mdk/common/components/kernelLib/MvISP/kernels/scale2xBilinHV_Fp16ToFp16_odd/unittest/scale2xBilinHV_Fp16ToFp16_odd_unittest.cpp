//Scale2xBilinHV_Fp16ToFp16_odd kernel test
// input interval is suposed to be subunitary

//Asm function prototype:
//    void mvispScale2xBilinHV_Fp16ToFp16_odd_asm(half **in, half **out, u32 width)

//Asm test function prototype:
//    void Scale2xBilinHV_Fp16ToFp16_odd_asm_test(half **in, half **out, unsigned int width);

//C function prototype:
//    void mvispScale2xBilinHV_Fp16ToFp16_odd(half **in, half **out, u32 width)
...
#include "scale2xBilinHV_Fp16ToFp16_odd_asm_test.h"
#include "scale2xBilinHV_Fp16ToFp16_odd_unittest.h"
...
#define PADDING 16
const half DELTA = 0.125f;

using ::testing::TestWithParam;
using ::testing::Values;

class scale2xBilinHV_Fp16ToFp16_oddTest : public ::testing::TestWithParam <unsigned long> {
protected:
...
TEST_F(scale2xBilinHV_Fp16ToFp16_oddTest, TestRandomInput_minus1_1_MaxWidth)
{
    width = 1920;
  ...
  inLinesOffseted = inputGen.GetOffsettedLines(inLines, 2, PADDING/2);  
    mvispScale2xBilinHV_Fp16ToFp16_odd(inLinesOffseted, outLineC, width);
    scale2xBilinHV_Fp16ToFp16_odd_asm_test(inLines, outLineAsm, width);

    CompareArrays(outLineC[0], outLineAsm[0], width, DELTA);    
}

//------------ parameterized tests ----------------------------------------
INSTANTIATE_TEST_CASE_P(RandomInputs, scale2xBilinHV_Fp16ToFp16_oddTest,
        Values((unsigned int)8, (unsigned int)16, (unsigned int)32, (unsigned int)48, (unsigned int)240, (unsigned int)512, (unsigned int)880);
);

TEST_P(scale2xBilinHV_Fp16ToFp16_oddTest, TestRandomInputParameterizedWidth)
{
    width = GetParam() * 2;
  inputGen.SelectGenerator("random");

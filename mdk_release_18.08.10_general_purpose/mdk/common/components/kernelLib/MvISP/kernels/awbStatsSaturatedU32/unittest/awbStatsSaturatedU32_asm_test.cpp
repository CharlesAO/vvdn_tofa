...
void awbStatsSaturatedU32_asm_test(unsigned int *out, unsigned short *in, unsigned int nPaxelX, unsigned int gapPaxelX, unsigned int widthPaxelX, unsigned int satPaxelX, unsigned int width)
{
    ...
    genRunner.SetInput("output", (unsigned char *)out, (nPaxelX*2+16)*4, SHAVE0);

	genRunner.GuardInsert(string("output"), SHAVE0, (nPaxelX*2+16) * 4, (unsigned char *)out);
	...
	genRunner.GetOutput(string("output"), SHAVE0, (nPaxelX*2+16) * 4, (unsigned char *)out);
	genRunner.GuardCheck(string("output"), SHAVE0, (nPaxelX*2+16) * 4, (unsigned char *)out);
}

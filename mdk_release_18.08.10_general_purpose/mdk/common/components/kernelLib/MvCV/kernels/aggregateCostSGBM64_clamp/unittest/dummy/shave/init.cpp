...
int main( void )
{
	...
	//printf("%x %x \n", in1,out);
		...
		mvcvAggregateCostSGBM64_clamp(input1, input2, out, penaltyP1, penaltyP2, min2, activateClamp);
	#else
		mvcvAggregateCostSGBM64_clamp_asm(input1, input2, out, penaltyP1, penaltyP2, min2, activateClamp);
	//mvcvAggregateCostSGBM64_clamp(input1, input2, out, penaltyP1, penaltyP2, min2, activateClamp);
	#endif

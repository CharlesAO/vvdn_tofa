// convolution7x7s3hhhh kernel test
//Asm function prototype:
//     void Convolution7x7s3hhhh_asm(half** in, half** out, half conv[49], u32 inWidth);
//
//Asm test function prototype:
//     void Convolution7x7s3hhhh_asm_test(half** in, half** out, half conv[49], unsigned int inWidth);
//
//C function prototype:
//     void Convolution7x7s3hhhh(half** in, half** out, half conv[49], u32 inWidth);
//
...
#include "convolution7x7s3hhhh_asm_test.h"
...
class Convolution7x7s3hhhhKernelTest : public ::testing::TestWithParam< unsigned int > {
protected:
...
TEST_F(Convolution7x7s3hhhhKernelTest, TestUniformInputLinesMinimumWidth)
{
    width = 32;
    ...
    Convolution7x7s3hhhh_asm_test(inLines, outLinesAsm, convMat, width);
    RecordProperty("CyclePerPixel", convolution7x7s3hhhhCycleCount / (width / 3));

    inLines = inputGen.GetOffsettedLines(inLines, SIZE, PADDING / 2);
    mvcvConvolution7x7s3hhhh(inLines, outLinesC, convMat, width);

    outputChecker.CompareArrays(outLinesC[0], outLinesAsm[0], width / 3, DELTA);
}

TEST_F(Convolution7x7s3hhhhKernelTest, TestUniformInputLinesAll0)
{
    width = 640;
    inputGen.SelectGenerator("uniform");
...
TEST_F(Convolution7x7s3hhhhKernelTest, TestUniformInputLinesAll255)
{
    width = 160;
    inputGen.SelectGenerator("uniform");
...
TEST_F(Convolution7x7s3hhhhKernelTest, TestUniformInputLinesRandomConvMatrix)
{
    width = 640;
    inputGen.SelectGenerator("uniform");
...
TEST_F(Convolution7x7s3hhhhKernelTest, TestRandomInputLinesRandomConvMatrix)
{
    width = 320;
    inputGen.SelectGenerator("random");
...
INSTANTIATE_TEST_CASE_P(UniformInputs, Convolution7x7s3hhhhKernelTest,
...
TEST_P(Convolution7x7s3hhhhKernelTest, TestRandomInputLinesBlurFilter)
{

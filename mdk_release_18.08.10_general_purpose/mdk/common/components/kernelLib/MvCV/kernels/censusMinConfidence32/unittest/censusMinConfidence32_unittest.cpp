...
#include "censusMinConfidence32_asm_test.h"
...
class mvcvCensusMinConfidence32KernelTest : public ::testing::TestWithParam< unsigned int > {
protected:
...
TEST_F(mvcvCensusMinConfidence32KernelTest, TestWidth16All0)
{
    width = 16;
    disparities = 32;
    ...
    mvcvCensusMinConfidence32((unsigned char *)&inLine[16], (unsigned char *)&minPosC[16],(unsigned char *)&minValC[16], width);
    mvcvCensusMinConfidence32_asm_test(inLine , minPosAsm, minValAsm, width);
    
    RecordProperty("CyclePerPixel", mvcvCensusMinConfidence32CycleCount / (width ));
    outputChecker.CompareArrays(minValC, minValAsm, width + PADDING);
    outputChecker.CompareArrays(minPosC, minPosAsm, width + PADDING);
}

TEST_F(mvcvCensusMinConfidence32KernelTest, TestWidth16All255)
{
    width = 16;
    disparities = 32;
    inputGen.SelectGenerator("uniform");
...
TEST_F(mvcvCensusMinConfidence32KernelTest, TestMinWidth)
{
    width = 8;
    disparities = 32;
    inputGen.SelectGenerator("uniform");
...
TEST_F(mvcvCensusMinConfidence32KernelTest, TestRandomDataSmallWidth)
{
...
TEST_F(mvcvCensusMinConfidence32KernelTest, TestRandomDataBigWidth)
{

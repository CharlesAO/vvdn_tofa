...
///
/// @brief     Convolution3x3 kernel function call for unitary test
///
...
u32 width;
half conv[9] __attribute__ ((aligned (16)));
...
u8* inLine[TEST_FRAME_HEIGHT] __attribute__ ((aligned (16)));
u8* outLine[TEST_FRAME_HEIGHT] __attribute__ ((aligned (16)));
u8** inLines;
u8** outLines;
...
int main( void )
{
	...
	mvcvConvolution3x3(inLines, outLines, conv, width);
	#else
		#ifdef MOVICOMPILE_OPTIMIZED
		mvcvConvolution3x3_opt(inLines, outLines, conv, width);
		#else
		mvcvConvolution3x3_asm(inLines, outLines, conv, width);
		#endif
	#endif

...
u32 prev[TEST_FRAME_WIDTH] __attribute__ ((aligned (16)));
...
int main( void )
{
	...
	mvcvIntegralimage_sum_u32_M2(output, input, runNr, previsionOutLnPointer,  width);
	#else
	mvcvIntegralimage_sum_u32_M2_asm(output, input, runNr, previsionOutLnPointer,  width);
	#endif

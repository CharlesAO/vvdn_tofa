...
int main( void )
{
	out = (u8 *)output + OFFSET;
		...
		mvcvComputeADPyrOnePos((unsigned char *)(&input1[OFFSET]), (unsigned char *)(&input2[OFFSET]), out, width);
	#else
		mvcvComputeADPyrOnePos_asm((unsigned char *)(&input1[OFFSET]), (unsigned char *)(&input2[OFFSET]), out, width);
	#endif

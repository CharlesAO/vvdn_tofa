...
protected:
    ...
    unsigned int *asmHist;
    unsigned int *cHist;
    ArrayChecker outputChecker;

    void cumulateHist(unsigned cHist[256], int width)
    {
        for( int i = 1; i <= 256; i++)
...
TEST_F(EqualizeHistTest, TestAsmAllValues0)
{
    ...
    asmHist = (unsigned int *)inputGen.GetLine(256 * 4, 0);
    cHist = (unsigned int *)inputGen.GetLine(256 * 4, 0);

    mvcvHistogram(inLines, cHist, 320);
    mvcvHistogram(inLines, asmHist, 320);
    cumulateHist(cHist, 16);
    cumulateHist(asmHist, 16);
    
    equalizeHist_asm_test(inLines, outLinesAsm, asmHist, 320);
    mvcvEqualizeHist(inLines, outLinesC, cHist, 320);
    
    RecordProperty("CycleCount", equalizeHistCycleCount);
        
    EXPECT_EQ((unsigned int)255, outLinesC[0][0]);
    EXPECT_EQ((unsigned int)255, outLinesC[0][100]);
    EXPECT_EQ((unsigned int)255, outLinesC[0][319]);
    EXPECT_EQ((unsigned int)255, outLinesAsm[0][0]);
    EXPECT_EQ((unsigned int)255, outLinesAsm[0][100]);
    EXPECT_EQ((unsigned int)255, outLinesAsm[0][319]);
...
TEST_F(EqualizeHistTest, TestAsmLowestLineSize)
{
    ...
    mvcvHistogram(inLines, cHist, 16);
    mvcvHistogram(inLines, asmHist, 16);
    cumulateHist(cHist, 16);
    cumulateHist(asmHist, 16);
    
    equalizeHist_asm_test(inLines, outLinesAsm, asmHist, 16);
    mvcvEqualizeHist(inLines, outLinesC, cHist, 16);
    
    RecordProperty("CycleCount", equalizeHistCycleCount);

    EXPECT_EQ((unsigned int)2, cHist[1]);
    EXPECT_EQ((unsigned int)10, cHist[18]);
    EXPECT_EQ((unsigned int)16, cHist[255]);
    EXPECT_EQ((unsigned int)2, asmHist[1]);
    EXPECT_EQ((unsigned int)10, asmHist[18]);
    EXPECT_EQ((unsigned int)16, asmHist[255]);
    EXPECT_EQ((unsigned int)31, outLinesAsm[0][0]);
    EXPECT_EQ((unsigned int)159, outLinesAsm[0][1]);
    EXPECT_EQ((unsigned int)223, outLinesAsm[0][15]);
    EXPECT_EQ((unsigned int)31, outLinesC[0][0]);
    EXPECT_EQ((unsigned int)159, outLinesC[0][1]);
    EXPECT_EQ((unsigned int)223, outLinesC[0][15]);
...
TEST_F(EqualizeHistTest, TestWidth240RandomData)
{
    ...
    mvcvHistogram(inLines, cHist, width);
    mvcvHistogram(inLines, asmHist, width);
    cumulateHist(cHist, 16);
    cumulateHist(asmHist, 16);
    
    equalizeHist_asm_test(inLines, outLinesAsm, asmHist, width);
    mvcvEqualizeHist(inLines, outLinesC, cHist, width);
...
TEST_F(EqualizeHistTest, TestAsmTwoLines)
{
    ...
    mvcvHistogram(&inLines[0], cHist, 16);
    mvcvHistogram(&inLines[0], asmHist, 16);
    mvcvHistogram(&inLines[1], cHist, 16);
    mvcvHistogram(&inLines[1], asmHist, 16);
    cumulateHist(cHist, 16);
    cumulateHist(asmHist, 16);
    
    equalizeHist_asm_test(&inLines[0], &outLinesAsm[0], asmHist, 16);
    equalizeHist_asm_test(&inLines[1], &outLinesAsm[1], asmHist, 16);
    mvcvEqualizeHist(&inLines[0],  &outLinesC[0], cHist, 16);
    mvcvEqualizeHist(&inLines[1],  &outLinesC[1], cHist, 16);
    
    RecordProperty("CycleCount", equalizeHistCycleCount);
 
    EXPECT_EQ((unsigned int)27, cHist[0]);
    EXPECT_EQ((unsigned int)27, asmHist[0]);
    EXPECT_EQ((unsigned int)32, cHist[255]);
    EXPECT_EQ((unsigned int)32, asmHist[255]);
    EXPECT_EQ((unsigned int)255, outLinesAsm[0][0]);
    EXPECT_EQ((unsigned int)223, outLinesAsm[0][1]);
    EXPECT_EQ((unsigned int)215, outLinesAsm[0][15]);
    EXPECT_EQ((unsigned int)215, outLinesAsm[1][0]);
    EXPECT_EQ((unsigned int)255, outLinesAsm[1][1]);
    EXPECT_EQ((unsigned int)239, outLinesAsm[1][2]);
    EXPECT_EQ((unsigned int)255, outLinesC[0][0]);
    EXPECT_EQ((unsigned int)223, outLinesC[0][1]);
    EXPECT_EQ((unsigned int)215, outLinesC[0][15]);
    EXPECT_EQ((unsigned int)215, outLinesC[1][0]);
    EXPECT_EQ((unsigned int)255, outLinesC[1][1]);
    EXPECT_EQ((unsigned int)239, outLinesC[1][2]);

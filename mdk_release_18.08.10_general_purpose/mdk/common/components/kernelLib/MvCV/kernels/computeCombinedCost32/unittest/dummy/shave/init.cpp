...
int main( void )
{
	//printf("%x %x \n", in1,out);
		...
		mvcvComputeCombinedCost32(disparityCost, adCost, alpha, beta, width, normFactor);
	#else
		mvcvComputeCombinedCost32_asm(disparityCost, adCost, alpha, beta, width, normFactor);
	//mvcvComputeCombinedCost32(disparityCost, adCost, alpha, beta, width, normFactor);
	#endif

...
#include "convolution3x3s3hhhh_asm_test.h"
...
class Convolution3x3s3hhhhKernelTest : public ::testing::TestWithParam< unsigned int > {
protected:
...
TEST_F(Convolution3x3s3hhhhKernelTest, TestRandomInputLinesRandomConvMatrix)
{
	width =32;// randGen->GenerateUInt(0, 1921, 8);
	...
	mvcvConvolution3x3s3hhhh(inLinesOffseted, outLinesC, convMat, width);
	Convolution3x3s3hhhh_asm_test(inLines, outLinesAsm, convMat, width);
	RecordProperty("CyclePerPixel", Convolution3x3s3hhhhCycleCount / width);

	outputChecker.CompareArrays(outLinesC, outLinesAsm, (int)(width/3), 1, DELTA);
...
TEST_F(Convolution3x3s3hhhhKernelTest, TestExperimental)
{
	width = 1920;
	inputGen.SelectGenerator("random");
	...
	Convolution3x3s3hhhh_asm_test(inLines, outLinesAsm, convMatAsm, width);
	RecordProperty("CyclePerPixel", Convolution3x3s3hhhhCycleCount / width);

	inLines = inputGen.GetOffsettedLines(inLines, SIZE, PADDING / 2);
	mvcvConvolution3x3s3hhhh(inLines, outLinesC, convMatC, width);

	outputChecker.CompareArrays(outLinesC, outLinesAsm, width/3, 1, DELTA);
}

TEST_F(Convolution3x3s3hhhhKernelTest, TestUniformInputLinesWidth16)
{
	width = 16;
	...
	Convolution3x3s3hhhh_asm_test(inLines, outLinesAsm, convAsm, width);
	RecordProperty("CyclePerPixel", Convolution3x3s3hhhhCycleCount / width);

	inLines = inputGen.GetOffsettedLines(inLines, SIZE, PADDING / 2);
	mvcvConvolution3x3s3hhhh(inLines, outLinesC, convC, width);

	outputChecker.ExpectAllInRange<half>(outLinesAsm[0], width/3, 2, 3);
	outputChecker.CompareArrays(outLinesC, outLinesAsm, width/3, 1, DELTA);
}


TEST_F(Convolution3x3s3hhhhKernelTest, TestUniformInputLinesAllValues255)
{
	width = 640;
	inputGen.SelectGenerator("uniform");
...
TEST_F(Convolution3x3s3hhhhKernelTest, TestRandomLinesSobel)
{
	width =64;
	inputGen.SelectGenerator("random");
	inLines = inputGen.GetLinesFloat16(width + PADDING, 3, 0, 255);
...
TEST_F(Convolution3x3s3hhhhKernelTest, TestRandomLinesEdgeDetect)
{
	width = 640;
	inputGen.SelectGenerator("random");
	inLines = inputGen.GetLinesFloat16(width + PADDING, 3, 0, 255);
...
TEST_F(Convolution3x3s3hhhhKernelTest, TestRandomLinesSharpen)
{
	width = 640;
	inputGen.SelectGenerator("random");
	inLines = inputGen.GetLinesFloat16(width + PADDING, 3, 0, 255);
...
TEST_F(Convolution3x3s3hhhhKernelTest, TestRandomLinesRandomKernel)
{
	width = 640;
	inputGen.SelectGenerator("random");
	inLines = inputGen.GetLinesFloat16(width + PADDING, 3, 0, 255);
...
INSTANTIATE_TEST_CASE_P(UniformInputs, Convolution3x3s3hhhhKernelTest,
...
TEST_P(Convolution3x3s3hhhhKernelTest, TestRandomInputLinesEmbossFilter)
{

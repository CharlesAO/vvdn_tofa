...
u8 src[MAX_FRAME_SIZE] __attribute__ ((aligned (16)));

/*output pre pad marker*/
uint32_t  __attribute__((section(".kept.data"))) dst_u32prePad[4] __attribute__ ((aligned (16)));
/*output data marker*/
u8 __attribute__((section(".kept.data"))) __attribute__ ((aligned (16))) dst[MAX_FRAME_SIZE] ;
/*output post pad marker*/
uint32_t __attribute__((section(".kept.data"))) __attribute__ ((aligned (16))) dst_u32postPad[4] ;
...
//	void mvcvBilinearInterpolation16u(const u8* src, s32 src_step, u8* dst, s32 dst_step, ClSize win_size, mvCvPoint2D32f center)
	...
	center.y = center_fp32[1];
		...
		mvcvBilinearInterpolationAligned16u(src, src_step, dst, dst_step, win_size, center);
	#else
		mvcvBilinearInterpolationAligned16u_asm(src, src_step, dst, dst_step, win_size, center);
	#endif

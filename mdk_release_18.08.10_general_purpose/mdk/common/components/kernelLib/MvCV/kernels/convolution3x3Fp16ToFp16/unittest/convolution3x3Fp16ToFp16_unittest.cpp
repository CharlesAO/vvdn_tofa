...
#include "convolution3x3Fp16ToFp16_asm_test.h"
...
class Convolution3x3Fp16ToFp16KernelTest : public ::testing::TestWithParam< unsigned int > {
protected:
...
TEST_F(Convolution3x3Fp16ToFp16KernelTest, TestRandomInputLinesRandomConvMatrix)
{
	...
	mvcvConvolution3x3Fp16ToFp16(inLinesOffseted, outLinesC, convMat, width);
	Convolution3x3Fp16ToFp16_asm_test(inLines, outLinesAsm, convMat, width);
	RecordProperty("CyclePerPixel", convolution3x3Fp16ToFp16CycleCount / width);
...
TEST_F(Convolution3x3Fp16ToFp16KernelTest, TestExperimental)
{
	width = 1920;
	inputGen.SelectGenerator("random");
	...
	Convolution3x3Fp16ToFp16_asm_test(inLines, outLinesAsm, convMatAsm, width);
	RecordProperty("CyclePerPixel", convolution3x3Fp16ToFp16CycleCount / width);

	inLines = inputGen.GetOffsettedLines(inLines, SIZE, PADDING / 2);
	mvcvConvolution3x3Fp16ToFp16(inLines, outLinesC, convMatC, width);
	
	outputChecker.CompareArrays(outLinesC, outLinesAsm, width, 1, DELTA);
}

TEST_F(Convolution3x3Fp16ToFp16KernelTest, TestUniformInputLinesWidth16)
{
	width = 16;
	...
	Convolution3x3Fp16ToFp16_asm_test(inLines, outLinesAsm, convAsm, width);
	RecordProperty("CyclePerPixel", convolution3x3Fp16ToFp16CycleCount / width);
	
	inLines = inputGen.GetOffsettedLines(inLines, SIZE, PADDING / 2);
	mvcvConvolution3x3Fp16ToFp16(inLines, outLinesC, convC, width);
...
TEST_F(Convolution3x3Fp16ToFp16KernelTest, TestUniformInputLinesAllValues255)
{
	width = 640;
	inputGen.SelectGenerator("uniform");
...
TEST_F(Convolution3x3Fp16ToFp16KernelTest, TestRandomLinesSobel)
{
	width =64;
	inputGen.SelectGenerator("random");
	inLines = inputGen.GetLinesFloat16(width + PADDING, 3, 0, 255);
...
TEST_F(Convolution3x3Fp16ToFp16KernelTest, TestRandomLinesEdgeDetect)
{
	width = 640;
	inputGen.SelectGenerator("random");
	inLines = inputGen.GetLinesFloat16(width + PADDING, 3, 0, 255);
...
TEST_F(Convolution3x3Fp16ToFp16KernelTest, TestRandomLinesSharpen)
{
	width = 640;
	inputGen.SelectGenerator("random");
	inLines = inputGen.GetLinesFloat16(width + PADDING, 3, 0, 255);
...
TEST_F(Convolution3x3Fp16ToFp16KernelTest, TestRandomLinesRandomKernel)
{
	width = 640;
	inputGen.SelectGenerator("random");
	inLines = inputGen.GetLinesFloat16(width + PADDING, 3, 0, 255);
...
INSTANTIATE_TEST_CASE_P(UniformInputs, Convolution3x3Fp16ToFp16KernelTest,
...
TEST_P(Convolution3x3Fp16ToFp16KernelTest, TestRandomInputLinesEmbossFilter)
{

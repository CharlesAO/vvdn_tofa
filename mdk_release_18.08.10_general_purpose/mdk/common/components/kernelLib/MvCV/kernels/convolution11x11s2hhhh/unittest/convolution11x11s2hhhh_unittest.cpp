//convolution11x11s2hhhh kernel test
//Asm function prototype:
//     void Convolution11x11s2hhhh_asm(half** in, half** out, half conv[][11], u32 inWidth);
//
//Asm test function prototype:
//     void Convolution11x11s2hhhh_asm(half** in, half** out, half conv[][11], unsigned int inWidth);
//
//C function prototype:
//     void Convolution11x11s2hhhh(half** in, half** out, half conv[][11], u32 inWidth);
//
...
#include "convolution11x11s2hhhh_asm_test.h"
...
class Convolution11x11s2hhhhKernelTest : public ::testing::TestWithParam< unsigned int > {
protected:
...
TEST_F(Convolution11x11s2hhhhKernelTest, TestUniformInputLinesMinimumWidth)
{
	...
	Convolution11x11s2hhhh_asm_test(inLines, outLinesAsm, convMat, width);
	RecordProperty("CyclePerPixel", convolution11x11s2hhhhCycleCount / (width/4));

        
	inLines = inputGen.GetOffsettedLines(inLines, SIZE, PADDING / 2);
	mvcvConvolution11x11s2hhhh(inLines, outLinesC, convMat, width);
...
TEST_F(Convolution11x11s2hhhhKernelTest, TestUniformInputLinesAll0)
{
	width = 640;
	inputGen.SelectGenerator("uniform");
...
TEST_F(Convolution11x11s2hhhhKernelTest, TestUniformInputLinesAll255)
{
	width = 640;
	inputGen.SelectGenerator("uniform");
...
TEST_F(Convolution11x11s2hhhhKernelTest, TestUniformInputLinesRandomConvMatrix)
{
	width = 64;
	inputGen.SelectGenerator("uniform");
...
TEST_F(Convolution11x11s2hhhhKernelTest, TestRandomInputLinesRandomConvMatrix)
{   
	width = 16;
	inputGen.SelectGenerator("random");
...
INSTANTIATE_TEST_CASE_P(UniformInputs, Convolution11x11s2hhhhKernelTest,
...
TEST_P(Convolution11x11s2hhhhKernelTest, TestRandomInputLinesBlurFilter)
{

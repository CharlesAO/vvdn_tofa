//convolution5x5s8hhhh kernel test
//Asm function prototype:
//     void Convolution5x5s8hhhh_asm(half** in, half** out, half conv[][3], u32 inWidth);
//
//Asm test function prototype:
//     void Convolution5x5s8hhhh_asm(half** in, half** out, half conv[][3], unsigned int inWidth);
//
//C function prototype:
//     void Convolution5x5s8hhhh(half** in, half** out, float conv[][3], u32 inWidth);
//
...
#include "convolution5x5s8hhhh_asm_test.h"
...
class Convolution5x5s8hhhhKernelTest : public ::testing::TestWithParam< unsigned int > {
protected:
...
TEST_F(Convolution5x5s8hhhhKernelTest, TestUniformInputLinesWidth16)
{   
	width = 16;
	inputGen.SelectGenerator("uniform");
	inLines =inputGen.GetLinesFloat16 (width + PADDING, 5, 3);
	outLinesC = inputGen.GetEmptyLinesFp16(width/8, 1);
	outLinesAsm = inputGen.GetEmptyLinesFp16(width/8, 1);

	inputGen.FillLine(convMat, 25, 0.04f);
		
	Convolution5x5s8hhhh_asm_test(inLines, outLinesAsm, convMat, width);
	RecordProperty("CyclePerPixel", convolution5x5s8hhhhCycleCount / (width/8));

	inLines = inputGen.GetOffsettedLines(inLines, SIZE, PADDING / 2);
	mvcvConvolution5x5s8hhhh(inLines, outLinesC, convMat, width);
...
TEST_F(Convolution5x5s8hhhhKernelTest, TestMinimumWidth)
{   
	width = 8;
	inputGen.SelectGenerator("uniform");
...
TEST_F(Convolution5x5s8hhhhKernelTest, TestSobelFilter)
{
	width = 80;
...
TEST_F(Convolution5x5s8hhhhKernelTest, TestEmbossFilter)
{
	width = 320;
	inputGen.SelectGenerator("random");
...
INSTANTIATE_TEST_CASE_P(RandomInputs, Convolution5x5s8hhhhKernelTest,
...
TEST_P(Convolution5x5s8hhhhKernelTest, TestRandomInputLinesBlurFilter)
{
...
TEST_P(Convolution5x5s8hhhhKernelTest, TestRandomInputLinesRandomMatrix)
{
	unsigned int width = GetParam();
	inputGen.SelectGenerator("random");

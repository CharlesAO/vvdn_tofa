//convolution3x3s2hhhh kernel test
//Asm function prototype:
//     void mvcvConvolution3x3s2hhhh_asm(half** in, half** out, half conv[9], u32 inWidth);
//
//Asm test function prototype:
//     void Convolution3x3s2hhhh_asm(half** in, half** out, half conv[9], unsigned int inWidth);
//
...
#include "convolution3x3s2hhhh_asm_test.h"
...
class Convolution3x3s2hhhhKernelTest : public ::testing::TestWithParam< unsigned int > {
protected:
...
TEST_F(Convolution3x3s2hhhhKernelTest, TestRandomInputLinesRandomConvMatrix)
{
    ...
    mvcvConvolution3x3s2hhhh(inLinesOffseted, outLinesC, convMat, width);
    Convolution3x3s2hhhh_asm_test(inLines, outLinesAsm, convMat, width);
    RecordProperty("CyclePerPixel", convolution3x3s2hhhhCycleCount / (width/2));
...
TEST_F(Convolution3x3s2hhhhKernelTest, TestExperimental)
{
    width = 1920;
    inputGen.SelectGenerator("random");
    ...
    Convolution3x3s2hhhh_asm_test(inLines, outLinesAsm, convMatAsm, width);
    RecordProperty("CyclePerPixel", convolution3x3s2hhhhCycleCount / (width/2));

    inLines = inputGen.GetOffsettedLines(inLines, SIZE, PADDING / 2);
    mvcvConvolution3x3s2hhhh(inLines, outLinesC, convMatC, width);

    outputChecker.CompareArrays(outLinesC, outLinesAsm, width/2, 1, DELTA);
}

TEST_F(Convolution3x3s2hhhhKernelTest, TestUniformInputLinesWidth16)
{
    width = 16;
    ...
    Convolution3x3s2hhhh_asm_test(inLines, outLinesAsm, convAsm, width);
    RecordProperty("CyclePerPixel", convolution3x3s2hhhhCycleCount / (width/2));

    inLines = inputGen.GetOffsettedLines(inLines, SIZE, PADDING / 2);
    mvcvConvolution3x3s2hhhh(inLines, outLinesC, convC, width);
...
TEST_F(Convolution3x3s2hhhhKernelTest, TestUniformInputLinesAllValues255)
{
    width = 640;
    inputGen.SelectGenerator("uniform");
    ...
    //outputChecker.ExpectAllInRange<half>(outLinesAsm[0], width/2, 254, 255);
    outputChecker.CompareArrays(outLinesC, outLinesAsm, width/2, 1, DELTA);
}

//------------ test different convolution matrices ------------------------

TEST_F(Convolution3x3s2hhhhKernelTest, TestRandomLinesSobel)
{
    width =64;
    inputGen.SelectGenerator("random");
    inLines = inputGen.GetLinesFloat16(width + PADDING, 3, 0, 255);
...
TEST_F(Convolution3x3s2hhhhKernelTest, TestRandomLinesEdgeDetect)
{
    width = 640;
    inputGen.SelectGenerator("random");
    inLines = inputGen.GetLinesFloat16(width + PADDING, 3, 0, 255);
...
TEST_F(Convolution3x3s2hhhhKernelTest, TestRandomLinesSharpen)
{
    width = 640;
    inputGen.SelectGenerator("random");
    inLines = inputGen.GetLinesFloat16(width + PADDING, 3, 0, 255);
...
TEST_F(Convolution3x3s2hhhhKernelTest, TestRandomLinesRandomKernel)
{
    width = 640;
    inputGen.SelectGenerator("random");
    inLines = inputGen.GetLinesFloat16(width + PADDING, 3, 0, 255);
...
INSTANTIATE_TEST_CASE_P(UniformInputs, Convolution3x3s2hhhhKernelTest,
...
TEST_P(Convolution3x3s2hhhhKernelTest, TestRandomInputLinesEmbossFilter)
{

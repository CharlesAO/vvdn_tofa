...
//     void mvcvExtractDescriptor64_freakA_asm(unsigned short *in_pixel_array,  unsigned char *out_descriptor);
//
...
#include "extractDescriptor64_freakA_asm_test.h"
...
class extractDescriptor64_freakAKernelTest : public ::testing::TestWithParam< unsigned int > {
protected:
...
TEST_F(extractDescriptor64_freakAKernelTest, TestUniformInputLines32)
{
	...
	mvcvExtractDescriptor64_freakA(inLine, outLineC);
	extractDescriptor64_freakA_asm_test(inLine, outLineAsm);

	RecordProperty("CyclePerPixel", extractDescriptor64_freakACycleCount);

	outputChecker.CompareArrays(outLineC, outLineAsm, 64, DELTA);
}


TEST_F(extractDescriptor64_freakAKernelTest, TestAllValuesZero)
{
	inputGen.SelectGenerator("uniform");
...
TEST_F(extractDescriptor64_freakAKernelTest, TestAllValues255)
{
	inputGen.SelectGenerator("uniform");
...
TEST_F(extractDescriptor64_freakAKernelTest, TestRandomWidthRandomData)
{

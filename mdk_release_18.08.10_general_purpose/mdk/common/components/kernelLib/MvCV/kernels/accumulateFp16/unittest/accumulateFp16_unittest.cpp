...
//    void mvcvAccumulateFp16_asm(half** dst, half** input, u32 width);
//
...
#include "accumulateFp16_asm_test.h"
...
#define PADDING 32
class AccumulateFp16Test : public ::testing::Test {
 protected:
...
TEST_F(AccumulateFp16Test , AllUniform)
{
    width = 16;
    ...
    AccumulateFp16_asm_test(outLinesASM, inLines, width);
    mvcvAccumulateFp16(outLinesC, inLines, width);
    RecordProperty("CycleCount", AccumulateFp16CycleCount);

    arrCheck.CompareArrays(outLinesASM[0],outLinesC[0],width + PADDING);
}

TEST_F(AccumulateFp16Test , AllUniform0)
{
    width = 1024;
    inputGen.SelectGenerator("uniform");
...
TEST_F(AccumulateFp16Test , AllUniform255)
{
    width = 1024;
    inputGen.SelectGenerator("uniform");
...
TEST_F(AccumulateFp16Test , AllRandomSmallWidth)
{
...
TEST_F(AccumulateFp16Test , AllRandomBigWidth)
{
...
TEST_F(AccumulateFp16Test , AllRandomMaxWidth)
{
    width = 1920;
    inputGen.SelectGenerator("random");
    inLines = inputGen.GetLinesFloat16(width + PADDING, 1, 0, 255);

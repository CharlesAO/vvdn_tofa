...
/*output data marker*/
u8 __attribute__((section(".kept.data"))) output_tile[16 * 16 ] __attribute__ ((aligned (16)));
/*output post pad marker*/
uint32_t __attribute__((section(".kept.data"))) outputtile_u32postPad[4] __attribute__ ((aligned (16)));

u8  input_tile[TILE_HEIGHT * TILE_WIDTH] __attribute__ ((aligned (16)));
...
int main( void )
{
	//printf("%x %x\n", input_tile, output_tile);
		...
		mvcvtile_sampleru8(input_tile, output_tile, xcoord, ycoord, tile_width, tile_height);
	#else
		mvcvtile_sampleru8_asm(input_tile, output_tile, xcoord, ycoord, tile_width, tile_height);
	#endif

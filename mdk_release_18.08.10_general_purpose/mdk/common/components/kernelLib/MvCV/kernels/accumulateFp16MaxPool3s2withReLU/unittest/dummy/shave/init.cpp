...
///
/// @brief     accumulateFp16MaxPool3s2withReLU kernel function call for unitary test
///
...
int main( void )
{
	...
	outLines = (half**)outLine;
		...
		mvcvAccumulateFp16MaxPool3s2withReLU(outLines, inLines, width);
	#else 
	#ifdef MOVICOMPILE_OPTIMIZED
		mvcvAccumulateFp16MaxPool3s2withReLU_opt(outLines, inLines, width);
		#else  
		mvcvAccumulateFp16MaxPool3s2withReLU_asm(outLines, inLines, width);
		#endif
	#endif
	SHAVE_HALT;
    return 0;

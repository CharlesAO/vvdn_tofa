...
#include "computeCombinedCost3_asm_test.h"
...
class mvcvComputeCombinedCost3KernelTest : public ::testing::TestWithParam< unsigned int > {
protected:
...
TEST_F(mvcvComputeCombinedCost3KernelTest, TestAll0)
{
	width = 320;	
	...
	mvcvComputeCombinedCost3(disparityCostC, adCost, alpha, beta, width, normFactor);
	mvcvComputeCombinedCost3_asm_test(disparityCostAsm, adCost, alpha, beta, width, normFactor);
	RecordProperty("CyclePerPixel", mvcvComputeCombinedCost3CycleCount / (width * DISPARITIES ));

	outputChecker.CompareArrays(disparityCostC, disparityCostAsm, width * DISPARITIES, DELTA);
}

TEST_F(mvcvComputeCombinedCost3KernelTest, TestAllMAX)
{
	width = 1280;
	inputGen.SelectGenerator("random");
	alpha  = randGen->GenerateUInt(1, 5, 1);
...
TEST_F(mvcvComputeCombinedCost3KernelTest, TestRandomDdataRandomWidth)
{
...
TEST_F(mvcvComputeCombinedCost3KernelTest, TestRandomDataRandomWidth2)
{

...
///
/// @brief     accumulateFp16MaxPool2s2 kernel function call for unitary test
///
...
int main( void )
{
	...
	outLines = (half**)outLine;
		...
		mvcvAccumulateFp16MaxPool2s2(outLines, inLines, width);
	#else   
		mvcvAccumulateFp16MaxPool2s2_asm(outLines, inLines, width);
	#endif

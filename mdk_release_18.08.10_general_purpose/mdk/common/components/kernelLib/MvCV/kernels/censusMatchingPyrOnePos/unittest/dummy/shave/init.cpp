...
/*output data marker*/
unsigned char  __attribute__((section(".kept.data"))) output[TEST_FRAME_WIDTH*3]__attribute__ ((aligned (16)));
...
int main( void )
{
	...
	//printf("%x %x %x %x \n", in1,in2, predicted,out);
	#ifdef UNIT_TEST_USE_C_VERSION
		mvcvCensusMatchingPyrOnePos(in1,in2,predicted,out,width);
	#else
		mvcvCensusMatchingPyrOnePos_asm(in1,in2,predicted,out,width);
	#endif

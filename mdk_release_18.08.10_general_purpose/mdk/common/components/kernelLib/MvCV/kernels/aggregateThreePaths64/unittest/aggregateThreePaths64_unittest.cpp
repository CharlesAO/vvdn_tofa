...
// mvcvAggregateThreePaths64 	- computes average sum from 4 paths (width x 64 arrays)
...
#include "aggregateThreePaths64_asm_test.h"
...
class mvcvAggregateThreePaths64KernelTest : public ::testing::TestWithParam< unsigned int > {
protected:
...
TEST_F(mvcvAggregateThreePaths64KernelTest, TestThree0)
{
	width = 320;
	...
	mvcvAggregateThreePaths64(path0, path1, path2, outputC + OFFSET, width);
	mvcvAggregateThreePaths64_asm_test(path0, path1, path2, outputAsm, width);
	
	RecordProperty("CyclePerPixel", mvcvAggregateThreePaths64CycleCount / (width * DISPARITIES ));

	outputChecker.CompareArrays(outputC, outputAsm, width * DISPARITIES + PADDING);
}

TEST_F(mvcvAggregateThreePaths64KernelTest, TestThreeMaxValues)
{
	width = 320;
	inputGen.SelectGenerator("uniform");
	outputC   = inputGen.GetEmptyLine(width * DISPARITIES + PADDING);
...
TEST_F(mvcvAggregateThreePaths64KernelTest, TestThreeRandomValues)
{
	width = 320;
	inputGen.SelectGenerator("uniform");
	outputC   = inputGen.GetEmptyLine(width * DISPARITIES + PADDING);

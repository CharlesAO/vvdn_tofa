...
#include "warpMeshSample8bit_asm_test.h"
...
class warpMeshSample8bittest : public ::testing::Test {
 protected:
...
TEST_F(warpMeshSample8bittest , dummy_test)
{
	...
	out_tileC   = (unsigned char *)inputGen.GetLine((TILE_SIZE * TILE_SIZE), 0);
    out_tileAsm = (unsigned char *)inputGen.GetLine((TILE_SIZE * TILE_SIZE), 0);
	...
	my   = inputGen.GetLineFloat16(TILE_SIZE * (TILE_SIZE + PADDING), 0.0);
		 ...
		 mvcvWarpMeshSample8bit(tile,   out_tileC, mx, my, tile_width, tile_height);
	warpMeshSample8bit_asm_test(tile, out_tileAsm, mx, my, tile_width, tile_height);
	...
	RecordProperty("CyclePerPixel", warpMeshSample8bitCycleCount / (TILE_SIZE * TILE_SIZE));

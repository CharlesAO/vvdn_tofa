...
//    void mvcvMinMaxPos_asm(u8** in, u32 width, u32 height, u8* minVal, u8* maxVal,u8* minPos, u8* maxPos, u8* maskAddr);
//
...
//    void mvcMinMaxKernel(u8** in, u32 width, u32 height, u8* minVal, u8* maxVal, u8* maskAddr);
//
...
TEST_F(MinMaxPosTest, TestUniformInputLine)
{
	...
	mvcvMinMaxPos(inputLines, width, &cMinVal, &cMaxVal, &cMinPos, &cMaxPos,mask);
...
TEST_F(MinMaxPosTest, TestAsmAllValues255)
{
	...
	EXPECT_EQ(cMinVal, asmMinVal);
	EXPECT_EQ(cMaxVal, asmMaxVal);
	EXPECT_EQ(cMinPos, asmMinPos);
	EXPECT_EQ(cMaxPos, asmMaxPos);
...
TEST_F(MinMaxPosTest, TestLefmostMinRightmostMax) 
{
	width = 320;
	inputGen.SelectGenerator("uniform");
	inputLines = inputGen.GetLines(width, 1, 5);
	mask = inputGen.GetLine(width, 1);

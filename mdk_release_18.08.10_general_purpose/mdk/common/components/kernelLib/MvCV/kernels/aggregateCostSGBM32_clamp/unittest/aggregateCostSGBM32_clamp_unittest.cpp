...
// mvcvAggregateCostSGBM32clampkernel 	- computes aggregated cost over a given path considering min2, penaltyP1 and penaltyP2 previously computed
...
#include "aggregateCostSGBM32_clamp_asm_test.h"
...
class mvcvAggregateCostSGBM32_clampKernelTest : public ::testing::TestWithParam< unsigned int > {
protected:
...
TEST_F(mvcvAggregateCostSGBM32_clampKernelTest, TestAll0)
{
	penaltyP1 = 0;
	penaltyP2 = 0;
	min2 = 0;
	...
	mvcvAggregateCostSGBM32_clamp(input1, input2, outputC + OFFSET, penaltyP1, penaltyP2, min2, activateClamp);
	mvcvAggregateCostSGBM32_clamp_asm_test(input1, input2, outputAsm, penaltyP1, penaltyP2, min2, activateClamp);
	
	RecordProperty("CyclePerPixel", mvcvAggregateCostSGBM32clampCycleCount / (WIDTH ));
	outputChecker.CompareArrays(outputC, outputAsm, WIDTH + PADDING);
}

TEST_F(mvcvAggregateCostSGBM32_clampKernelTest, TestAllBigValues)
{
	penaltyP1 = 2000; 
	penaltyP2 = 2000; 
	min2 = 255; 
...
TEST_F(mvcvAggregateCostSGBM32_clampKernelTest, TestAllRandomValues)
{
	inputGen.SelectGenerator("uniform");
	outputC   = inputGen.GetEmptyLine(WIDTH + PADDING);
    outputAsm = inputGen.GetEmptyLine(WIDTH + PADDING);

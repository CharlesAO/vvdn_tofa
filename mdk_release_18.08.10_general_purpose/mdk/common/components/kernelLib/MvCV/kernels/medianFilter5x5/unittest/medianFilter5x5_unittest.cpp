...
//	void mvcvMedianFilter5x5_asm(u32 widthLine, u8 **outLine, u8 ** inLine);

//Asm test function prototype:
// 	void medianFilter5x5_asm_test(unsigned char width, unsigned char **outLine, unsigned char **inLine);

//C function prototype:
// 	void mvcvMedianFilter5x5(u32 widthLine, u8 **outLine, u8 ** inLine);
...
TEST_F(medianFilter5x5Test, TestUniformInputLines32)
{
	width = 32;
	...
	mvcvMedianFilter5x5(width, outLinesC, inLinesOffseted);
	medianFilter5x5_asm_test(width, outLinesAsm, inLines);
	RecordProperty("CyclePerPixel", medianFilter5x5CycleCount / width);
	
	outputCheck.CompareArrays(outLinesC[0], outLinesAsm[0], width);
}

TEST_F(medianFilter5x5Test, TestAllValuesZero)
{
	width = 320;
	unsigned char **inLinesOffseted;
	inputGen.SelectGenerator("uniform");
...
TEST_F(medianFilter5x5Test, TestAllValues255)
{
	width = 320;
	unsigned char **inLinesOffseted;
	inputGen.SelectGenerator("uniform");
...
TEST_F(medianFilter5x5Test, TestMinimumWidthSize)
{
	width = 16;
	unsigned char **inLinesOffseted;
	inputGen.SelectGenerator("uniform");
...
TEST_F(medianFilter5x5Test, TestSimpleNotEqualInputLines)
{
	width = 320;
	unsigned char **inLinesOffseted;
	inputGen.SelectGenerator("uniform");
...
TEST_F(medianFilter5x5Test, TestRandomWidthRandomData)
{
	unsigned char **inLinesOffseted;

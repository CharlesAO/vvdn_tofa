...
//!@{
/// tile_samplerRGB888 - in order to compute everything ok we need one extra pixel to the right and one extra line at the bottom for interpolation
/// @param[in] tile                 - Input tile of data, (u16 format) it has variable width/height size
/// @param[out] out_tile            - Output tile (u16 format) it has a 16*3*16 fixed dimension ; 3 is because we have RGB interlieved
...
MVCV_FUNC(void, mvcvtile_samplerRGB888, u8 *tile, u8 * out_tile, float *xcoord, float *ycoord, u32 tile_width, u32 tile_height)

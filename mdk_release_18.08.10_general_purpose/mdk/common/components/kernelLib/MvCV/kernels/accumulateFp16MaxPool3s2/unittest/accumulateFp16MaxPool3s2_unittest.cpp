//AccumulateFp16MaxPool3s2 test    
//Asm function prototype:
//    void mvcvAccumulateFp16MaxPool3s2_asm(half** dst, half** input, u32 width);
//
...
#include "accumulateFp16MaxPool3s2_asm_test.h"
...
#define PADDING 32
class AccumulateFp16MaxPool3s2Test : public ::testing::Test {
 protected:
...
TEST_F(AccumulateFp16MaxPool3s2Test , AllUniform)
{
    width = 16;
    ...
    AccumulateFp16MaxPool3s2_asm_test(outLinesASM, inLines, width);
    mvcvAccumulateFp16MaxPool3s2(outLinesC, inLines, width);
    RecordProperty("CycleCount", AccumulateFp16MaxPool3s2CycleCount);

    arrCheck.CompareArrays(outLinesASM[0],outLinesC[0],width + PADDING);
}

TEST_F(AccumulateFp16MaxPool3s2Test , AllUniform0)
{
    width = 1024;
    inputGen.SelectGenerator("uniform");
...
TEST_F(AccumulateFp16MaxPool3s2Test , AllUniform255)
{
    width = 1024;
    inputGen.SelectGenerator("uniform");
...
TEST_F(AccumulateFp16MaxPool3s2Test , AllRandomSmallWidth)
{
...
TEST_F(AccumulateFp16MaxPool3s2Test , AllRandomBigWidth)
{
...
TEST_F(AccumulateFp16MaxPool3s2Test , AllRandomMaxWidth)
{
    width = 1920;
    inputGen.SelectGenerator("random");
    inLines = inputGen.GetLinesFloat16(width + PADDING, 1, 0, 255);

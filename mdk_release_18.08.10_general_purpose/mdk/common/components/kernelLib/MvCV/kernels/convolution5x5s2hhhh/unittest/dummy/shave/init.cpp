...
///
/// @brief     Convolution5x5s2hhhh kernel function call for unitary test
///
...
/*output data marker*/
half __attribute__((section(".kept.data"))) output[TEST_FRAME_HEIGHT][TEST_FRAME_WIDTH/2]__attribute__ ((aligned (16)));
...
int main( void )
{
    ...
    outLines    = (half**)outLine;
        ...
        mvcvConvolution5x5s2hhhh(inLines, outLines, conv, width);
   #else
        mvcvConvolution5x5s2hhhh_asm(inLines, outLines, conv, width);
    #endif

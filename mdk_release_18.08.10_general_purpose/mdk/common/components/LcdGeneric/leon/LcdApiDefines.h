...
///
/// @defgroup LcdApiDefines LCD API Defines
/// @ingroup  LcdGeneric
/// @{
...
/// structures, enums and exported variables for the LCD Generic component
///
...
//dynamic IRQ's used
#define ROUTED_IRQ_LCD    IRQ_DYNAMIC_3
...
/// LCD needs to be aware of the TX datapath
typedef enum {
    LCD_TO_PARALLEL = 0,       /// If LCD output goes to parallel port
    LCD_TO_MIPI,               /// If LCD output goes to mipi 
    LCD_INVALID_OUTPUT         /// List terminator = used for sanity checks
}lcdDatapath_t;
...
struct _lcdHandle
{
    ...
    lcdDatapath_t           outputDataPath;    /// Where LCD outputs the data to
};

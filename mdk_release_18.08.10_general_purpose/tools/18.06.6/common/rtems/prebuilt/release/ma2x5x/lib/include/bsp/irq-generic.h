 ...
 */
  ...
  {
    /**
    * Notice for future users: the following return is correct considering that
    * rtems_vector_number is generally an unsigned int for most architectures.
    * However, in case this is reused or branched into an architecture where this
    * may not be unsigned int, but a signed one instead the following return should change to:

...
#else
#define MOVIDEBUG2_CDECL __attribute__ (( __cdecl__ ))
#endif
...
#define MOVIDEBUG_OPT_TCF_TRANSPORT_URL "tcf:url"
#define MOVIDEBUG_OPT_TCF_DISCOVERY "tcf:discovery"
#define MOVIDEBUG_OPT_DDR_INIT_ELF "ddrinit:elf"
#define MOVIDEBUG_OPT_VECTOR_REGISTERS "tcf:shave-vector-registers"
#define MOVIDEBUG_OPT_RTEMS_THREADS "tcf:rtems-threads"
#define MOVIDEBUG_OPT_LEON_RT_SUPPORT "tcf:leon-rt-support"
#define MOVIDEBUG_OPT_SINGLE_LEON_RT "tcf:single-leon-rt"
#define MOVIDEBUG_OPT_LEON_NN_SUPPORT "tcf:leon-nn-support"
#define MOVIDEBUG_OPT_SHAVES_SUPPORT "tcf:shaves-support"
#define MOVIDEBUG_OPT_SHAVE_NN_SUPPORT "tcf:ucnn-support"
#define MOVIDEBUG_OPT_SHAVE_L2_CACHE_DIRECT_MODE_AS_DDR "tcf:shave-l2-cache-direct-mode-as-ddr"

#define MOVIDEBUG_OPT_TCF_CORE_STATE_UPDATE "tcf:core-state-update"
#define MOVIDEBUG_OPT_UART "tcl:uart"

#define MOVIDEBUG_OPT_MVPROTO_PROCESSOR_INDEX "mvproto::processor-index"
#define MOVIDEBUG_OPT_MVPROTO_CLIENT_BUSY_MESSAGE "mvproto:client-busy-message"
#define MOVIDEBUG_OPT_MVPROTO_SERVER_MESSAGE_VERBOSITY "mvproto:message-verbosity"

#define MOVIDEBUG_OPTVAL_TRUE  "true"
#define MOVIDEBUG_OPTVAL_FALSE "false"
#define MOVIDEBUG_OPTVAL_OFF   "off"
#define MOVIDEBUG_OPTVAL_ON    "on"

#define MOVIDEBUG_OPTVAL_SILENT  "silent"
#define MOVIDEBUG_OPTVAL_DEBUG   "debug"
#define MOVIDEBUG_OPTVAL_VERBOSE "verbose"
#define MOVIDEBUG_OPTVAL_INFO    "info"
#define MOVIDEBUG_OPTVAL_WARNING "warning"
#define MOVIDEBUG_OPTVAL_ERROR   "error"
...
#define MOVIDEBUG_DLL_TCF_RESPONSE_TYPE_ERROR 'E'
#define MOVIDEBUG_DLL_TCF_RESPONSE_TYPE_RESULT 'R'

typedef struct MoviDebugTcfResponse
{
    /// Should be 0 in case of successful call.
    int error;
    /// ResponseType is either 'E' or 'R'
    /// \note it's not `char` to avoid packing issues
    int type;
    /// Array of response items (null-terminated strings). Last pointer is NULL.
    char * * data;
    /// Number of response items. Makes life easier.
    size_t size;
    /// Use this function to free up dynamicaly allocated structure
    void (MOVIDEBUG2_CDECL *deallocate)(struct MoviDebugTcfResponse *self);

} MoviDebugTcfResponse;
...
typedef struct MoviDebugDllInterface
{
    ...
    /**
     *  Send TCF command <service, command, data> and wait for response asynchronously
     *  The caller needs to call the deallocate() method on the result to free up resources.
     */
    MoviDebugTcfResponse* (MOVIDEBUG2_CDECL *tcfSendCommand)(MoviDebugObjectPtr dbg, const char *service, const char *command, const void *data, size_t size);
    ...
    int (MOVIDEBUG2_CDECL *symSize)(MoviDebugObjectPtr dbg, const char *target, const char *name, MoviDebugDllSize *result);
